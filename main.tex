...
\usepackage{amsfonts, mathtools, multirow}
...
\chapter{New approach to reduce private keys}

The contents of this chapter are an extended version of the work published as~\cite{Zambonin:201907}. We observe that the reuse of vinegar variables in the signature generation step of the Rainbow scheme leads to a shorter representation of its central map, and thus, of the entire private key. We analyse the security implications of this strategy and present a modification to the Rainbow scheme, enabling a private key size reduction of up to $85\%$ with secure parameters. Additionally, this framework can be applied on top of already existing schemes that shorten either private or public keys, spawning derivatives that reduce the total key pair size by a factor of 3.5.

\section{Our proposal}\label{sec:proposal}

We will describe our improvement to Rainbow-like signature schemes below, as
...
the addition of $w$. Real implementations of Rainbow-$\eta_{3}$ are tested on
...
matrix with elements in $\mathbb{F}$ is invertible.

Assume a square matrix $M$ of order $n$ such that
$m_{ij} \in \mathbb{F}_{q}, i, j \in \{1, \dots, n\}$. For $M$ to be
invertible, it must be composed entirely of vectors, \emph{i.e.} its rows
$m_{i} \in \mathbb{F}^{n}$, that are linearly independent. The zero vector
$(0, \dots, 0) \in \mathbb{F}^{n}$ is linearly dependent of all other vectors.
Thus, $m_{1} \neq (0, \dots, 0)$, with all other $q^{n} - 1$ possible vectors
eligible. $m_{2}$ must not feature any of the $q$ multiples of $m_{1}$, and
...
other words, all square matrices of size $v_{i}, i \in \{1, \dots, u\}$ need to
...
will denote this common case as $\Pi(q, n, 2) = \widetilde{\Pi}(q, n)$. Note
...
we choose $n \in \{56, \dots, 90\}$
from~\cite[Tables 6.4, 6.8, 6.13]{Petzoldt:201307:phd} and calculate the
...
\begin{figure}[htbp]
  ...
  \caption{Probability of obtaining an invertible matrix, populated with field
    elements where $q \in \{2, \dots 256\}$ and $q$ is a prime power, given the
    quantity of layers of Rainbow.}\label{fig:1}
\end{figure}

It is also useful to calculate $\lim_{n \to \infty} \Pi(q, n)$ to observe
changes in the probability with the growth of $m$. Note that this is very
similar to the Euler function $\phi(q)$. Ergo, we can use one of Euler's
identities to redefine the above limit as
\begin{align}
...
We use the SageMath language arbitrary precision real numbers to obtain these
values and find out that, when $n \geq 56$,
...
amortised by the overwhelming probability that a signature is successfully
generated.
...
probability, where the first $v_{1}$ elements of any $y$ will be the same.
...
When part of the vector $y$ is fixed, we will instead denote these by
$\widetilde{Z}'_{t}$ and $\widetilde{Z}_{t}$. Our hypothesis is that $Z_{t}$
and $\widetilde{Z}_{t}$ will behave similarly to observations sampled from the
...
is equal to $\sqrt{\frac{r^{2} - 1}{12}}$. For a finite field $\mathbb{F}$, we
set $r = q$ and obtain the desired value. It is expected that
\begin{align}
...
\begin{figure}[ht]
  ...
  \caption{Difference of standard deviations when $t \in \{1, \dots, 1024\}$,
    and $q \in \{2, \dots, 256\}$ with $q$ as a prime power.}\label{fig:2}
\end{figure}
...
fields of higher orders and lower $t$. For instance, given the finite field
...
differences of means, defined analogously as $d_{\mu}^{t}$. The field
...
for each class $k \in \{0, \dots, q - 1\}$ and refer to them by $Z_{t, k}$ and
...
by the Q-Q plot created with rankits, where the points are sufficiently close
to the $y = x$ expected line.
...
schemes if parameters are not chosen carefully. We will briefly state each of
those, along with their estimated complexities~\cite{Petzoldt:201005:inproc},
...
created by Kipnis and Shamir~\cite{Kipnis:199808:inproc} to break the Balanced
...
the map $\mathcal{T}$. The complexity of the generalised attack for unbalanced
schemes~\cite{Kipnis:199904:inproc} is
...
Rainbow~\cite{Billet:200609:inproc}, its complexity is
...
complexity of the improved attack~\cite{Ding:200806:inproc} is
$q^{o_{u}} \cdot \frac{n^{3}}{6}$ field multiplications.

\subsubsection{Rainbow-Band-Separation attack.} An extension of the
UOV-Reconciliation attack by the same authors~\cite{Ding:200806:inproc} that
...
\paragraph{} We do not discard the possibility that specialised attacks exist,
...
\emph{e.g.} according to~\cite{Petzoldt:201005:inproc}, to protect the scheme
...
Rainbow~\cite{Petzoldt:201005:inproc}. We implement several known guidelines
...
the Rainbow submission authors~\cite{Ding:201712:misc} to fit security
categories as requested by NIST\@. We apply our method to these recent
proposals, and additionally
choose parameters from Petzoldt~\cite[Table 6.12]{Petzoldt:201307:phd} for
...
possible to maximise the results of our method. However, we remark that one
...
optimisations of the private key often through the inclusion of inner
...
To the best of our knowledge, the schemes presented in
Subsection~\ref{subsec:related} feature changes that target the whole private
key evenly. Hence, our method would yield similar results to those in
Table~\ref{tab:1} if this assumption is true. However, it is also the case that
some variants were subsequently broken or new parameters were suggested. We
will thus consider only schemes that reduce the public key size, \emph{i.e.}
CyclicRainbow~\cite{Petzoldt:201012:inproc} and
RainbowLRS2~\cite[Section 9.2]{Petzoldt:201307:phd}.
...
parameters from Petzoldt~\cite[Table 9.8]{Petzoldt:201307:phd}. We calculate
$|\mathcal{K}_{Pu}|$ for the variants according to Equations 9.2 and 9.4 of the
same work, and as per its Remark 9.1, note that $q = 16$ and $q = 31$ are not
...
the key pair will be reduced more aggressively. Thus, our results reflect
