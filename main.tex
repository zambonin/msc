...
\begin{definition}
  Given a set of mathematical expressions $A$ with $a_{0}, \dots, a_{n} \in A$ as \emph{coefficients} and a \emph{variable} $x$, the expression $\sum_{i = 0}^{n} a_{i} x^{i} = a_{n} \cdot x^{n} + \dots + a_{2} \cdot x^{2} + a_{1} \cdot x + a_{0}$, is an \emph{univariate polynomial}.
\end{definition}
