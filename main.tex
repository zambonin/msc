...
Let $r \in \mathbb{N}$ be a precision parameter. A number $r \times q$ of samples are taken from a standard normal distribution and organized into $q$-uples $K_{0}, \dots, K_{r}$. Then, let $\min_{i}(S)$ represent the $i$-th smallest value in a set $S$. The $q$-uple $K$ representing the approximate normal order statistics is calculated as
\[
    K = (\mu(\min_{i}(K_{j})): 1 \leq i \leq q).
\]
