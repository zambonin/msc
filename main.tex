...
\begin{align}
  ...
  \end{split}
\end{align}
The bipolar construction can be extended to work differently from the main idea given, for instance in the case of schemes that perform calculations over multiple finite fields. We invite the reader to consult~\cite[Section 2.2]{Petzoldt:201307} for a discussion of schemes that do not employ the bipolar construction at all, or that use multiple finite fields in their construction.

 There exist signature schemes constructed via the Fiat--Shamir transformation which only depend on the polynomial system solving problem such as MQDSS~\cite{Chen:201612}, but their efficiency is lacking compared to schemes based on the bipolar construction.
...
Consequently, a scheme using the bipolar construction is also based on some variant of the \emph{isomorphism of polynomials (IP) problem}. There are several versions of this problem, and we give a short definition for the variant that Oil--Vinegar schemes are based on, as introduced in~\cite{Ding:200806}.
...
Unfortunately, there is little information on the difficulty of EIP~\cite[p. 69]{Thomae:201306}, preventing complete security proofs for most multivariate cryptosystems.
...
It is argued in~\cite[Lemmata 3.2, 3.10, 3.19]{Thomae:201306} that some IP-class problems are no harder to solve if $\mathcal{L}_{1}$ and $\mathcal{L}_{2}$ are linear instead of affine transformations. As a consequence, such results need the polynomials in the central map to be homogeneous quadratic. We make use of this technical result and tailor our description of the bipolar construction to be coherent with the schemes presented in Section~\ref{sec:ov}.
