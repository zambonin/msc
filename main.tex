...
There always exists at least one basis for a vector space~\cite{}. Indeed, it is a compact way of representing the entire set of vectors. For example, the \emph{standard basis} is a sequence of unit vectors $(1, 0, \dots, 0), \dots, (0, \dots, 0, 1)$ over $\mathbb{F}^{n}$, and its usual arrangement is the \emph{identity matrix}, hereafter denoted as $\mathbf{I}_{n}$. In fact, several structures previously defined can also be represented as matrices, which is useful when performing calculations with relatively abstract elements such as polynomials.

% invertible affine map -> invertible matrix
% topics for tomorrow: matrices, general linear group, agl, matrix representations of certain polynomials
