...
Our attack is inspired by the method of Kipnis--Shamir~\cite{Kipnis:199808}, which is a key recovery attack that separates oil and vinegar variables through manipulation of linear combinations of the public key. It is based on the fact that, in the case of the balanced Oil--Vinegar scheme, a mapping of the oil subspace $\mathcal{O} = \{ x_{1}, \dots, x_{n} \in \mathbb{F}^{n} : x_{1} = \cdots = x_{v} = 0 \}$ to itself can be found in polynomial time. It is discussed in more detail in Subsection~\ref{subsec:uov}.

The modification of Rainbow-$\eta$ is a fixation of vinegar variables in the signature generation step of the scheme. This is allowed, since the map can be reconstructed through either a seed, mathematical relations between the public key and the central map, as explained in Subsection~\ref{subsec:scheme}, or simply by changing the message with a nonce due to the EU-CMA trait. The difference between signature made with the same vinegar variables and usual signatures is not statistically significant, as argued in Subsection~\ref{subsec:similar}. Moreover, there is no limitation on parameters such that a secure selection is always allowed.
...
The construction of the basis $B$ for $\mathcal{L}_{2}^{-1}(\mathcal{O})$ is given below. A standard basis is taken as a blueprint, its last $m$ rows are substituted with the vectors in $\rho$, and the matrix is then transposed. Equivalently,
\begin{align*}
  ...
  \end{bmatrix}.
\end{align*}
Then, if $B \in \mathsf{GL}_{n}$, the inverse of $\widetilde{\mathcal{L}}_{2}$ can be constructed. If this is not the case, then other vectors in $\rho$ need to be picked. This is due to the fact that the desired equivalent private key is of the form $\mathbf{sk'} = (\mathbf{pk} \circ \widetilde{\mathcal{L}}_{2}, \widetilde{\mathcal{L}}_{2}^{-1})$. By representing $B$ as a linear transformation and performing the aforementioned operation, we obtain an equivalent key. Indeed, $\mathbf{pk} \circ \widetilde{\mathcal{L}}_{2}$ must be a system of Oil--Vinegar polynomials~\cite[Theorem 1]{Beullens:201706}. Finally, an attacker can use $\mathbf{sk'}$ and produce signature forgeries of any desired message.
