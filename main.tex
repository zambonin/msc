...
\begin{definition}
  ...
  \begin{itemize}
    \item It is \emph{associative} if we have that $g_{1} \star (g_{2} \star g_{3}) = (g_{1} \star g_{2}) \star g_{3}$;
    \item It is \emph{commutative} if we have that $g_{1} \star g_{2} = g_{2} \star g_{1}$;
    \item It is \emph{closed under $G$} if we have that $g_{1} \star g_{2} \in G$.
  \end{itemize}
\end{definition}
...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item The binary operation \emph{$\ast$ left- and right-distributes over $+$}, that is, we must have that $r_{1} \ast (r_{2} + r_{3}) = (r_{1} \ast r_{2}) + (r_{1} \ast r_{3})$ and $(r_{2} + r_{3}) \ast r_{1} = (r_{2} \ast r_{1}) + (r_{3} \ast r_{1})$.
  \end{enumerate}
\end{definition}
...
We hereafter refer to $0$ as the \emph{additive identity} and to $1$ as the \emph{multiplicative identity} of a ring, if it exists. Analogously, for any element $r$ of a ring, we denote $-r$ as the \emph{additive inverse of $r$}, and $r^{-1}$ as the \emph{multiplicative inverse of $r$}, if it exists. Indeed, these inverse binary operations are respectively the \emph{subtraction} and \emph{division}.
...
% \begin{definition}
%   Given a ring $(R, +, \ast)$ and any $r_{1}, r_{2} \in R, r_{1}, r_{2} \neq 0$, if $r_{1} \ast r_{2} \neq 0$, then the ring is an \emph{integral domain}.
% \end{definition}
...
\begin{definition}
  Given $k \in \mathbb{N}$, a set of expressions $A$ with $a_{0}, \dots, a_{k} \in A$ as \emph{coefficients} and an \emph{indeterminate} $x$, an expression $p(x)$ of the form
  \[
    \sum_{i = 0}^{k} a_{i} x^{i} 
...
Vector spaces are the object of study of linear algebra. The members of $V$ are \emph{vectors}, and the members of $\mathbb{F}$ are \emph{scalars}. It follows that the binary operation $\bm{\times}$ is the \emph{scalar multiplication}. If clear from the context, we hereafter name vector spaces by capital boldface letters, without mentioning the associated binary operations. Given $c \in \mathbb{N}$, if members of $V$ are $c$-tuples of elements of $\mathbb{F}$, the resulting vector space is a \emph{coordinate space} and usually represented by $\mathbb{F}^{c}$, with element-wise addition, subtraction and scalar multiplication. We can thus recall Definition~\ref{def:polsys} and create a function of several polynomials.
...
There always exists at least one basis for a vector space~\cite[p. 409, Prop. 1]{Dummit:2003}. Indeed, it is a compact way of representing the entire set of vectors. For example, the \emph{standard basis} is a sequence of unit vectors $(1, 0, \dots, 0), \dots, (0, \dots, 0, 1)$ over $\mathbb{F}^{n}$. The ordered arrangement of the standard basis yields the \emph{identity matrix}, hereafter denoted as $\mathbf{I}_{n}$. Furthermore, all bases have the same number of elements~\cite[p. 411, Cor. 4]{Dummit:2003}, which is the \emph{dimension} of the vector space, or $\dim(\cdot)$. 
...
\begin{definition}
  ...
  \begin{enumerate}
    \item The \emph{addition operation is preserved}, that is, we must have that $\phi(\mathbf{v_{1}} \bm{+} \mathbf{v_{2}}) = \phi(\mathbf{v_{1}}) \bm{+'} \phi(\mathbf{v_{2}})$;
    \item The \emph{scalar multiplication operation is preserved}, that is, we must have that $\phi(f_{1} \bm{\times} \mathbf{v_{1}}) = f_{1} \bm{\times'} \phi(\mathbf{v_{1}})$.
  \end{enumerate} 
\end{definition}

Such functions between coordinate spaces are usually represented by polynomial maps. A trivial example of a linear transformation is the identity function $\mathsf{id} : \mathbf{V} \to \mathbf{V}$ such that $\mathsf{id}(\mathbf{v_{1}}) = \mathbf{v_{1}}$. Moreover, if $\dim \mathbf{V} = \dim \mathbf{V'}$, there may exist an inverse function $\phi^{-1} : \mathbf{V'} \to \mathbf{V}$ such that $\phi \circ \phi^{-1} = \mathsf{id}$. We remark that Definition~\ref{def:morph} and Proposition~\ref{prop:autgroup} are analogous in the setting of vector spaces. Indeed, there exists an automorphism group $\mathsf{Aut}(\mathbf{V})$, which is the set of all invertible linear transformations carrying usual function composition. In this case, this group is more commonly known as the \emph{general linear group} $\mathsf{GL}(\mathbf{V})$.
...
The cryptographic hash function is the usual method of transforming a variable-length message into a fixed-length \emph{digest}. The set $X$ is usually all binary words $\{0, 1\}^{*}$, while $Y$ is usually $\mathbb{F}_{2}^{k}$ for some small $k \in \mathbb{N}$. A well-designed function outputs an unpredictable string with no apparent connection to the input, which changes completely even with the smallest correction applied to the original message. It is a very useful tool for checking the integrity of messages.

\begin{definition}
  Given $n \in \mathbb{N}$, any message $w \in \{0, 1\}^{*}$ and a $3$-tuple of probabilistic polynomial-time algorithms $(\textsc{Gen}, \textsc{Sig}, \textsc{Ver})$, it is a \emph{digital signature scheme} if it satisfies
  \begin{align}
...
This mathematical framework ensures authenticity and non-repudiation of the sender, and integrity of the message for a secure choice of the $3$-uple. The algorithm $\textsc{Gen}$ generates the key pair, two bit strings known as the signing key and verification key, or private and public keys, which are related in some way. The algorithm $\textsc{Sig}$ outputs a signature, usually represented by $\sigma$, uniquely determined by the combination of the private key and the message. The algorithm $\textsc{Ver}$ verifies if the message was signed correctly with corresponding key and outputs a truth value.
...
\begin{definition}\label{def:posso}
  Given a polynomial system of the form presented in Definition~\ref{def:polsys}, finding a solution vector $x_{1}, \dots, x_{n}$ is the \emph{polynomial system solving problem}.
\end{definition}

\begin{theorem}[{\cite[Appendix A]{Patarin:199711}}]
  Given a polynomial system of \emph{quadratic equations over $\mathbb{F}_{2}$} of the form presented in Definition~\ref{def:polsys}, finding a solution vector $x_{1}, \dots, x_{n}$ is NP-complete. This is the \emph{MQ problem}.
\end{theorem}
...
\begin{definition}\label{def:quadsys}
  ...
  \begin{align}
    ...
    \end{split}
  \end{align}
  The set of all such systems is denoted by $\mathsf{MQ}(n, m)$. If all linear and constant terms are zero, then the resulting system is \emph{homogeneous quadratic}, and the respective set is denoted by $\mathsf{MHQ}(n, m)$.
\end{definition}
...
\begin{definition}
  ...
  \begin{itemize}
    \item The ordering given by $\mathbf{x}^{\alpha} \prec \mathbf{x}^{\beta}$ if $\alpha_{i} \prec \beta_{i}$ with minimal $1 \leq i \leq n$ such that $\alpha_{i} \neq \beta_{i}$ is called \emph{lexicographic ordering}, or ``dictionary ordering'', and denoted as \textsf{lex};
    \item The ordering given by $\mathbf{x}^{\alpha} \prec \mathbf{x}^{\beta}$ if $\deg \mathbf{x}^{\alpha} \prec \deg \mathbf{x}^{\beta}$ or, as a tie-breaker, $\mathbf{x}^{\alpha} \prec_{\mathsf{lex}} \mathbf{x}^{\beta}$ is called \emph{graded lexicographic ordering}, and denoted as \textsf{grlex};
    \item The ordering given by $\mathbf{x}^{\alpha} \prec \mathbf{x}^{\beta}$ if $\deg \mathbf{x}^{\alpha} \prec \deg \mathbf{x}^{\beta}$ or, as a tie-breaker, the last non-zero element of $\alpha - \beta$ is negative is called \emph{graded reverse lexicographic ordering}, and denoted as \textsf{grevlex}.
  \end{itemize}
\end{definition}
...
\begin{definition}\label{def:mathom}
  ...
  \begin{align}
    ...
    \end{bmatrix}
  \end{align}
  such that $p^{(i)} = \mathbf{x} \cdot M(p^{(i)}) \cdot \mathbf{x}^{\textsf{T}}$ for $\mathbf{x} = (x_{1}, \dots, x_{n})$. Its \emph{symmetric} version is given by $\widetilde{M}(p^{(i)}) = M(p^{(i)}) + M(p^{(i)})^{\mathsf{T}}$.
\end{definition}
...
We now turn to the most commonly used framework in multivariate cryptography employed to build cryptosystems. It is called bipolar construction, due to its elegant twofold nature, which yields the possibility of building public-key encryption or signature schemes with the same idea. 

Consider $n, m \in \mathbb{N}$ the number of variables and equations as in Definition~\ref{def:quadsys}. Let $\overline{\mathsf{MHQ}}(n, m)$ be a class of structured homogeneous quadratic polynomial maps and $\mathcal{C} : \mathbb{F}^{n} \to \mathbb{F}^{m} \in \overline{\mathsf{MHQ}}(n, m)$. This is called the \emph{central map} and regarded as the trapdoor, that is, a function that is easy to compute but difficult to invert without particular information. Evidently, it shall be hidden to prevent signature forgery. Two \emph{lateral maps} $\mathcal{L}_{1} : \mathbb{F}^{n} \to \mathbb{F}^{n} \in \mathsf{GL}_{n}(\mathbb{F})$ and $\mathcal{L}_{2} : \mathbb{F}^{m} \to \mathbb{F}^{m} \in \mathsf{GL}_{m}(\mathbb{F})$ are used to hide the structure of $\mathcal{C}$, by creating the composition $\mathcal{P} = \mathcal{L}_{1} \circ \mathcal{C} \circ \mathcal{L}_{2}$ that has no apparent structure, making recovery of the central map hard.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \node (x) at (0, 0) {$\mathbf{y} \in \mathbb{F}^{m}$};
    \node (y) at (2.5, 0) {$\mathbf{x} \in \mathbb{F}^{n}$};
...
The private and public keys are respectively a $3$-tuple and the aforementioned composition, that is,
\begin{align}
  \begin{split}
    \mathbf{sk} &= (\mathcal{L}_{1}, \mathcal{C}, \mathcal{L}_{2})
      \:\quad \in \mathsf{GL}_{n}(\mathbb{F}) \times \overline{\mathsf{MHQ}}(n, m) \times \mathsf{GL}_{m}(\mathbb{F}), \\
    \mathbf{pk} &= \mathcal{L}_{1} \circ \mathcal{C} \circ \mathcal{L}_{2} 
      \quad \in \mathsf{MHQ}(n, m).
  \end{split}
\end{align}
This description is tailored for schemes that perform calculations over a single finite field, as is the case of the Oil--Vinegar family. However, the bipolar construction can be extended to work differently from the main idea given. We invite the reader to consult~\cite[Section 2.2]{Petzoldt:201307} for a discussion of schemes that do not employ the bipolar construction at all, or that use multiple finite fields in their construction.

While its construction is very elegant, the bipolar construction yields a situation in which its derived schemes are not exclusively based on the polynomial system solving problem of Definition~\ref{def:posso}. Instead, it is proven that there exist many private keys that compose a single public key~\cite{Wolf:201104}, and consequently, the theory of \emph{equivalent keys} has been used to attack several schemes, as seen in Section~\ref{sec:related}. This concept is better explained alongside specific schemes, and thus it is done on Section~\ref{sec:ov}.

Consequently, a scheme using the bipolar construction is also based on some variant of the \emph{isomorphism of polynomials (IP) problem}. There are several versions of this problem, and we give a short definition for the variant that Oil--Vinegar schemes are based on.

\begin{definition}
  Let $\mathbb{U} = \mathsf{GL}_{n}(\mathbb{F}) \times \overline{\mathsf{MHQ}}(n, m) \times \mathsf{GL}_{m}(\mathbb{F})$ be a set of ``secret keys''. Given a $3$-tuple $(\mathcal{L}_{1}, \mathcal{C}, \mathcal{L}_{2}) \in \mathbb{U}$ with $\mathcal{P} = \mathcal{L}_{1} \circ \mathcal{C} \circ \mathcal{L}_{2}$, finding a decomposition $\mathcal{P} = \overline{\mathcal{L}_{1}} \circ \overline{\mathcal{C}} \circ \overline{\mathcal{L}_{2}}$ such that $(\overline{\mathcal{L}_{1}}, \overline{\mathcal{C}}, \overline{\mathcal{L}_{2}}) \in \mathbb{U}$ is the \emph{extended isomorphism of polynomials (EIP) problem}.
\end{definition}

Unfortunately, there is little information on the difficulty of EIP~\cite[p. 69]{Thomae:201306}, preventing complete security proofs for most multivariate cryptosystems. There exist signature schemes constructed via the Fiat--Shamir transformation which only depend on the polynomial system solving problem such as MQDSS~\cite{Chen:201612}, but their efficiency is lacking compared to schemes based on the bipolar construction.

Finally, we remark that our description of the bipolar construction is slightly different from the literature, since our maps lie in more restricted sets. The usual description has $\mathcal{C} \in \mathsf{MQ}(n, m)$ and $\mathcal{L}_{1}, \mathcal{L}_{2} \in \mathsf{AGL}_{n}(\mathbb{F})$. This is the \emph{affine group}, which is composed of matrix-vector pairs $(M, \mathbf{y})$ that encode the affine transformation $M\mathbf{x} \bm{+} \mathbf{y}$. By setting the translation vector $\mathbf{y} = (0, \dots, 0)$, we get a linear transformation. 

It is argued in~\cite[Lemmata 3.2, 3.10, 3.19]{Thomae:201306} that the underlying IP problems are no harder to solve if $\mathcal{L}_{1}$ and $\mathcal{L}_{2}$ are linear instead of affine transformations. Moreover, such results imply that the polynomials in the central map must be homogeneous quadratic. We make use of this technical result and present a coherent description of the schemes in Section~\ref{sec:ov}.

% there is kind of an incongruency here because EIP with fully homogeneous is said to be easy but then the proof of thomae requires homogeneous but does not talk about EIP
...
Finally, denote an instance of the scheme by Rainbow$(\mathbb{F}_{q}, v_{1}, o_{1}, \dots, o_{u})$. Note that when $u = 1$, we get the UOV scheme. Measured in field elements, the size of a private key is the number of elements in the matrix representations of $\mathcal{S}$ and $\mathcal{T}$, added to the count of all valid monomial combinations from each layer of Oil--Vinegar polynomials as per Equation~\ref{eq:ov}. In other words,
\begin{align}
...
by the Q-Q plot, which in this case is more commonly referred to as a normal probability plot. It is built by pairing the normalized counts with quantiles from the standard normal distribution. Such quantiles are called normal order statistics, or rankits~\cite{Ipsen:194405}, which are hereafter referred to as $K$. 
...
of Gröbner basis, such as the Hybrid approach~\cite{}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{l*{5}{r}}
    \toprule
    Parameters & Variant & $\#\mathsf{sk}$ & $\#\mathsf{pk}^{\eta}$ & $\#\mathsf{pk}$ & Difference \\ \midrule
