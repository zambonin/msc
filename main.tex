...
% TODO redo paragraphs on public key strategies (cyclic, luov, bac)

In the case of modifications to the public key, there are mainly three approaches. The first strategy is based on the method of field lifting, where the resulting LUOV scheme~\cite{} has the central map and public key lifted to an extension field, such that coefficients of those polynomial systems are now smaller. This is a trade-off between public key size and signature size, but it is general enough to be compatible with other public key reduction strategies. LUOV has been submitted to the standardization process by NIST~\cite{}, where it was 
...
and can make use of the improvements to the public key given above. 
