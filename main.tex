...
We note that in the case of STS and TTS, reducing the private key was not the main intention of the authors, and it is thus not trivial to estimate such gains through direct comparison to Rainbow. On the other hand, with the general description of the scheme available~\cite{}, there have been several works which focus on the modification of Rainbow with the purpose of reducing key sizes. Additionally, this motivation stems from recent efforts to present quantum-safe cryptography as a valid alternative to traditional digital signature schemes~\cite{},. We discuss these ``newer generation''  works as follows.
