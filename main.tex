...
Consider a cryptographic hash function $\mathcal{H} : \{0, 1\}^{*} \to \mathbb{F}^{m}$ and any message $M \in \{0, 1\}^{*}$. To sign $M$, obtain the message digest $d = \mathcal{H}(M)$. The signature is the set of variables which yield the solution to the equation $\mathcal{P}(\mathbf{x}) = d$.
