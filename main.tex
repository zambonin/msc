...
\begin{definition}
  Given a finite field $\mathbb{F}$, a set $V$, a binary operation $+$ on $V$ and a binary operation $\cdot : \mathbb{F} \to V$, a \emph{vector space over $\mathbb{F}$} is an ordered $3$-uple $(V, +, \cdot)$ satisfying the following axioms:
