...
\begin{theorem}
  Let $p$ be a prime number and $n \in \mathbb{N}$. Then, there exists a finite field with $p^{n}$ elements. Such a finite field $\mathbb{F}_{p^{n}}$ is isomorphic to $\mathbb{F}_{p}[x] / (f)$ with $f \in \mathbb{F}_{p}[x]$ irreducible and $\deg(f) = n$.
\end{theorem}

The finite field $\mathbb{F}_{p^{n}}$ can be represented as the set of polynomials with coefficients in $\mathbb{F}_{p}$ and degree smaller than $n$ carrying the usual polynomial arithmetic operations modulo $f$. Finite fields of the form $\mathbb{F}_{2^{n}}$ are especially useful in computer science, given that coefficients are restricted to $\{0, 1\}$ and may be represented by bit arrays, with arithmetic done through logic operations.
