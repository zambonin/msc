...
\begin{definition}
  Given $n \in \mathbb{N}$, any message $w \in \{0, 1\}^{*}$, and three probabilistic polynomial-time algorithms $\textsc{Gen}(1^{n}) \mapsto (\mathbf{sk}, \mathbf{pk})$
  
  a $3$-tuple of probabilistic polynomial time algorithms $(\textsc{Gen}, \textsc{Sig}, \textsc{Ver})$, it is said to be a \emph{digital signature scheme} if it satisfies the following conditions:
