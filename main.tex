...
We now turn to the most used framework to build cryptosystems in multivariate cryptography. The \emph{bipolar construction}, due to its elegant twofold nature, yields the possibility of building public-key encryption or signature schemes with the same idea. We remark that, as a consequence of the degree choice for multivariate cryptography, this is the only possible arrangement of maps. In other words, there must be a single quadratic map hidden by linear maps such that the composition of the functions is still a quadratic map.
...
It is a direct consequence of this theorem that there exist at least as many equivalent keys as the possible choices for $\omega$. Moreover, 
