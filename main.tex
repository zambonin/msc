...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item There exists a uniquely determined element $h^{-1} \in G$, called \emph{the inverse of $g_{1}$}, such that $g_{1} \star g_{1}^{-1} = g_{1}^{-1} \star g_{1} = e$, and indeed $(g_{1}^{-1})^{-1} = g_{1}$.
  \end{enumerate}
\end{definition}
...
We will depart from using $e$ as the identity element, for it is not clear which operation it refers to. We hereafter refer to $0$ as the \emph{additive identity} and to $1$ as the \emph{multiplicative identity}. Analogous, instead of 
