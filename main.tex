...
Thus, the only remaining task is to build a basis $B$ of $\widetilde{\mathcal{O}}$, which has dimension $m$. We leave it as an open question to calculate a lower bound for the size of $\rho$, 

Therefore, we need at least $m + 1$ signatures to perform $m$ subtractions and obtain $\widetilde{\rho} = \{ \chi_{1}, \dots, \chi_{m} \}$.

for which $m$ linearly independent $\chi_{i}$ must be collected. We note that at least $m + 1$ signatures need to be collected to perform $m$ subtractions. As seen in Section~\ref{sec:stats}, signatures are statistically similar to random elements of the underlying vector space. Thus, it is expected with high probability that all $\chi_{i}$ are linearly independent.
