...
Consider a finite field $\mathbb{F}_{q}$ and $v, o \in \mathbb{N}$. These integers represent the quantity of vinegar and oil variables, respectively. Let $\mathbf{v} = (x_{1}, \dots, x_{v})$ and $\mathbf{o} = (x_{1}$ The total number of variables is $n = v + o$, and thus $\mathbf{x} = (x_{1}, \dots, x_{v}, x_{v + 1}, \dots, x_{v + o})$. More intuitively, they may be divided as $\mathbf{x} = (x_{1}, \dots, x_{v}, x'_{1}, \dots, x'_{o})$.

\begin{definition}
  Given a polynomial ring $\mathbb{F}_{q}[\mathbf{x}]$, an \emph{Oil--Vinegar polynomial} is a multivariate quadratic polynomial of the form
  \begin{align}
    \sum_{i = 1}^{v} \sum_{j = 1}^{v} \alpha x_{i} x_{j} 
      + \sum_{i = 1}^{v} \sum_{j = 1}^{o} \beta x_{i} x'_{j} 
      + \sum_{}
  \end{align}
  The set of all such polynomials is defined as $\mathsf{OV}(n, m)$.
\end{definition}
