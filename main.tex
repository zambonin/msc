...
However, it is argued in~\cite[Remark 3.1]{Wolf:201104} that due to the theory of equivalent keys, it is not relevant to the security of schemes based on the bipolar construction if 

It is argued in~\cite[Lemmata 3.2, 3.10, 3.19]{Thomae:201306} that some IP problems modelled to represent Oil--Vinegar schemes are no harder to solve if $\mathcal{L}_{1}$ and $\mathcal{L}_{2}$ are linear instead of affine transformations. However, as a consequence, the polynomials in the central map must be homogeneous quadratic. While there is sufficient evidence that homogeneous quadratic maps is sage both in UOV~\cite[Section 5]{Kipnis:199808}~\cite[Section 3.1]{Braeken:200502} and Rainbow~\cite[Section 3.1]{Ding:201901}, but that may not be the case with other schemes built upon the bipolar construction.

Our description is based on these technical results. There is further independent evidence that using linear transformations and quadratic maps is safe both


 tailored to be coherent with the schemes presented in Section~\ref{sec:ov}
