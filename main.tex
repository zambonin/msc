...
A vibrant discipline of public-key cryptography, signature schemes are often associated with digital systems, in which transit of sensitive messages is again expected to be secure. The initial public discovery of digital signature schemes is given in~\cite{rsa}, which is still widely used at the time of the writing. It is based on the difficulty of integer factorization. Other popular schemes, such as~\cite{eddsa}, are based on the difficulty of calculating a discrete logarithm. There is no known polynomial time algorithm to solve such problems on a classical computer.
...
\emph{Post-quantum cryptography} addresses this issue by defining public-key cryptosystems which are either not known to be affected by quantum computers, or provably so~\cite{Bernstein:2008}. Several distinct mathematical structures are used to compose computationally hard underlying problems. For instance, the difficulty of solving polynomial systems over multiple variables. Primitives that use this strategy populate the area known as multivariate public-key cryptography, and are extremely time- and space-efficient when operating over messages. However, public and private keys are defined by large polynomial systems with little opportunity for compression or sparseness.
