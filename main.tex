...
\begin{definition}
  Given two groups $\mathsf{G}$ and $\mathsf{H}$, if an homomorphism $\varphi : \mathsf{G} \to \mathsf{H}$ is a bijection, then it is invertible, or an \emph{isomorphism}, and. If $\mathsf{G} = \mathsf{H}$, $\varphi$ is an \emph{automorphism}.
\end{definition}
