...
\begin{definition}
  Given a vector space and $\mathbf{v_{1}}, \dots, \mathbf{v_{n}} \in V$, the set of all linear combinations $f_{1} \cdot \mathbf{v_{1}} \oplus \dots \oplus f_{n} \cdot \mathbf{v_{n}}$, where each $f_{i}, \dots, f_{n}$ is any element in $\mathbb{F}$, is called the \emph{linear span}.
\end{definition}

\begin{definition}
  Given a vector space and $B \subseteq V$, if the elements of $B$ are linearly independent and the linear span of $B$ is $V$, then $B$ is called a \emph{basis of the vector space}.
\end{definition}
