...
\begin{definition}
  Given two vector spaces $\mathbf{V} = (V, \bm{+}, \bm{\times}), \mathbf{V'} = (V', \bm{+'}, \bm{\times'})$ over the same field $\mathbb{F}$, any vectors $\mathbf{v_{1}}, \mathbf{v_{2}} \in V$, and a function $\varphi : \mathbf{V} \to \mathbf{V'}$, $\phi$ is a \emph{linear transformation} if it satisfies the following conditions:
