...
We observe the distribution of field elements in vectors after the final function application, that is, $\mathbf{z} = \mathcal{L}_{2}^{-1}(\mathbf{x})$.


Let
$Z'_{t} = (\mathbf{z}_{1}, \dots, \mathbf{z}_{t}) \random \mathbb{F}^{n},\; t \in \mathbb{N}$ be
a $t$-tuple acting as the control group of unmodified signatures, and its unpacked notation $(n \times t)$-tuple extension $$Z_{t} = (z_{1}^{1},
