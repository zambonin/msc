...
However, it is simple enough to modify the key and signature generation algorithms to generate either conventional or reduced private keys at the will of the signer. If an attacker has no access to the signing device, it is thus not possible to discern whether our optimization is used. Otherwise, several other side-channel attacks may be carried out, possibly foiling Rainbow implementations even if our method is never used. Let us then consider that a signer produces a number $N$ of signatures such that a percentage $0 \leq \alpha \leq 1$ of those are created with the same vinegar variables.
