...
We remark that a polynomial by itself is simply a mathematical expression, but they are commonly used to define \emph{polynomial functions}. In this case, the indeterminates are called the \emph{variables} of the function $p(\textbf{x})$ associated with the polynomial. The domain and codomain of such a function must be thus well-defined, for instance, by picking a field. Further, the solutions to the associated equation $p(\textbf{x}) = 0$, if they exist, are called the zeros of the function or the \emph{roots} of a polynomial. 
...
\begin{definition}\label{def:polsys}
  Given $m \in \mathbb{N}$, a \emph{polynomial system} is a simultaneous sequence of polynomial equations $p^{(1)}(\textbf{x}) = 0, \dots, p^{(m)}(\textsf{x}) = 0$, usually denoted
  \begin{align*}
    \begin{cases}
...
\begin{definition}
  Given $n, m \in \mathbb{N}$, a finite field $\mathbb{F}$ and $p^{(1)}, \dots, p^{(m)} \in \mathbb{F}[x_{1}, \dots, x_{n}]$, a \emph{polynomial map} is a function $j : \mathbb{F}^{n} \to \mathbb{F}^{m}$ defined by a system of polynomial functions
  \begin{align*}
    j(x_{1}, \dots, x_{n}) =
    \begin{cases}
      p^{(1)}(x_{1}, \dots, x_{n}) \\
...
Such functions between coordinate spaces are usually represented by polynomial maps. A trivial example of a linear transformation is the identity function $\textsf{id} : \mathbf{V} \to \mathbf{V}$ such that $\textsf{id}(\mathbf{v_{1}}) = \mathbf{v_{1}}$. Moreover, if $\dim \mathbf{V} = \dim \mathbf{V'}$, there may exist an inverse function $\varphi^{-1} : \mathbf{V'} \to \mathbf{V}$ such that $\varphi \circ \varphi^{-1} = \textsf{id}$. We remark that Definition~\ref{def:morph} and Proposition~\ref{prop:autgroup} are analogous in the setting of vector spaces. Indeed, there exists an automorphism group $\textsf{Aut}(\mathbf{V})$, which is the set of all invertible linear transformations carrying usual function composition. In this case, this group is more commonly known as the \emph{general linear group} $\mathsf{GL}(\mathbf{V})$.
...
With the pure mathematical machinery defined, we now introduce the building blocks to create a generic digital signature scheme based on multivariate polynomial equations. We recall that public keys in multivariate cryptography are polynomial systems. We start by introducing which polynomials are chosen for this effect and how they interact to hide secret information and create a cryptosystem.

Most modern multivariate cryptography schemes are based on multivariate quadratic equations over finite fields, that is, polynomial systems with $\deg(\cdot) = 2$. We discuss this choice in Subsection~\ref{}.

\begin{definition}
  Given $n, m \in \mathbb{N}$, a polynomial ring $\mathbb{F}_{q}[x_{1}, \dots, x_{n}]$ and 
  
  a finite field $\mathbb{F}_{q}$ and $\alpha_{ij}, \beta_{i}, \gamma \in \mathbb{F}_{q}$, a \emph{system of multivariate quadratic polynomials over $\mathbb{F}_{q}$} is defined as
  \begin{align}
    \begin{split}
      \begin{cases}
        p^{(1)}(x_{1}, \dots, x_{n}) = 
...
As mentioned in Section~\ref{sec:algebra}, division of polynomials in a multivariate polynomial ring is ambiguous if a structure is not given to order monomials. More precisely, quotients and remainders of a division between two multivariate polynomials depend on which one is considered the divisor, that is, one that represents the ``largest quantity''. Thus, we give a short discussion on how to order multivariate polynomials.

\begin{definition}
  Let $\mathbb{Z}^{n}$ be the set of $n$-uples of integers and any $\alpha, \beta, \gamma \in \mathbb{Z}^{n}$. Given a well-order $\lesssim$ in $\mathbb{Z}^{n}$ such that $\alpha + \gamma \lesssim \beta + \gamma$ if $\alpha \lesssim \beta$, a \emph{monomial ordering} is defined by letting $\mathbf{x}^{\alpha} = x_{1}^{\alpha_{1}} x_{2}^{\alpha_{2}} \cdots x_{n}^{\alpha_{n}} \in \mathsf{R}[\mathbf{x}]$.
\end{definition}

The definition above amounts to ordering monomials according to their associated $n$-uples of exponents, which is relatively intuitive. There are several examples of ordering strategies~\cite{}. For instance, lexicographic ordering, more commonly known as the ``dictionary ordering'', compares the exponents of $x_{1}$ and only if they are equal it compares exponents of $x_{2}$, and so on. Graded lexicographic ordering sums the exponents of all monomials and uses lexicographic ordering as the tie-breaker.

In the context of multivariate cryptography, monomial ordering is used to define matrix representations of multivariate polynomials, and to solve polynomial systems using Gröbner bases~\cite{}. 
