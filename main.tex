...
\begin{definition}
  Given a polynomial ring $\mathsf{R}[\mathbf{x}]$ and the set of monomials $\mathbf{x}^{\alpha} = x_{1}^{\alpha_{1}} x_{2}^{\alpha_{2}} \cdots x_{n}^{\alpha_{n}}$ with $\alpha$ an $n$-uple of non-negative integers, a \emph{monomial ordering} is a relation $\lesssim$
\end{definition}
