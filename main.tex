...
A vibrant discipline of public-key cryptography, signature schemes are often associated with digital systems, in which transit of sensitive messages is again expected to be secure. The initial public discovery of digital signature schemes is given in~\cite{rsa} and still widely used at the time of the writing. It is based on the difficulty of integer factorization. Other popular schemes, such as~\cite{eddsa}, are based on the difficulty of calculating a discrete logarithm. There is no known polynomial time algorithm to solve such problems on a conventional computer.
...
\emph{Post-quantum cryptography} addresses this issue by defining public-key cryptosystems which are either not known to be affected by quantum computers, or provably so~\cite{Bernstein:2008}. Several distinct mathematical structures are used to compose such underlying problems. While a recent discipline, post-quantum cryptography has generated considerable research effort. A noteworthy example is th


for instance the difficulty of solving polynomial systems over multiple variables. Primitives that use this strategy are altogether known as \textit{multivariate public-key cryptography}, or simply multivariate cryptography, and are extremely time- and space-efficient when operating over messages. 

Nonetheless, public and private keys are defined by large polynomial systems with little opportunity for compression or sparseness. Large key sizes frequently impose constraints on the use of signature schemes in devices of limited storage and processing power. In multivariate cryptography, keys are often orders of magnitude larger than the ones used in conventional signature schemes.

% Due to this fact, several distinct approaches to reduce the key sizes of multivariate public-key cryptosystems have been proposed in the literature.
...
%      define hypothesis and scope before searching the literature (outline and contributions too)
