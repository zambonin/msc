...
Consider a cryptographic hash function $\mathcal{H} : \{0, 1\}^{*} \to \mathbb{F}^{m}$, any message $M \in \{0, 1\}^{*}$, and obtain the message digest $d = \mathcal{H}(M)$. Compute $\mathbf{y} = \mathcal{L}_{1}^{-1}(d)$. To
generate $\mathcal{C}(\mathbf{x}) = \mathbf{y}$, every layer must be inverted recursively.
