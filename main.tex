...
The definition above amounts to ordering monomials according to their associated $n$-uples of exponents, which is relatively intuitive. There are several examples of ordering strategies~\cite{}. For instance, lexicographic ordering, more commonly known as the ``dictionary ordering'', compares the exponents of $x_{1}$ and only if they are equal it compares exponents of $x_{2}$, and so on. On the other hand, graded lexicographic ordering sums the exponents of all monomials and uses lexicographic ordering as the tie-breaker.
