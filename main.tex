...
The definition above amounts to ordering monomials according to their associated $n$-uples of exponents, which is relatively intuitive. There are several examples of ordering strategies~\cite{}. For instance, lexicographic ordering, more commonly known as the ``dictionary ordering'', the exponents of $x_{1}$ are compared and only if they are equal exponents of $x_{2}$ are compared, and so on. In graded lexicographic ordering, the monomial degree is used as first comparison and lexicographic ordering as tie-breaker. Finally, graded reverse lexicographic ordering is more commonly used for efficiency reasons~\cite{}. 
