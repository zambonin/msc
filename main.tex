...
The examples given are the most commonly discussed monomial orders, but for efficiency reasons, \textsf{grevlex} is usually employed in algorithms~\cite{}. In the context of multivariate cryptography, monomial ordering is used for instance to define matrix representations of multivariate polynomials, and to solve polynomial systems using Gröbner bases~\cite{}. Additionally, as seen in Section~\ref{sec:related}, the visualization and manipulation of multivariate polynomials as matrices is pivotal in understanding and even attacking some cryptosystems.
...
We note that the matrix representation of $\mathcal{P}$ itself is highly dependant on a monomial ordering~\cite{}. On the other hand, the matrix of Definition~\ref{def:matquad} does not necessarily need any ordering, but the resulting product may differ from the original polynomial in this aspect. We also show how to represent multivariate quadratic polynomials with no constant or linear terms, which are known not to interfere with the security of the underlying cryptosystem~\cite{}.
...
We remark that the symmetric matrix of Definition~\ref{def:mathom} has a diagonal of zeroes if the underlying field characteristic is even. We now turn to the most commonly framework in multivariate cryptography employed to build cryptosystems. It is called bipolar construction, due to its elegant twofold nature, which yields the possibility of building public-key encryption or signature schemes with the same idea. 
...
We now observe the relationship between $m$ and $n$ to understand how the bipolar construction works. The lateral maps need not be considered in this discussion due to their lack of structure. For $m \geq n$, the function $\mathcal{C}$ should be injective, to ensure that there exists a unique plaintext $\mathbf{z}$ related to each ciphertext $\mathbf{h}$, by the preimage step. Indeed, there are public-key encryption schemes in which a non-negligible probability of failure in the decryption process exists~\cite{}. In the case of $m \leq n$, one expects that $\mathcal{C}$ is surjective, such that every message $\mathbf{h}$ has a signature $\mathbf{z}$, once again by the central map preimage.

The private and public keys are respectively a $3$-tuple and the aforementioned composition
\begin{align}
  \begin{split}
  ...
  \end{split}
\end{align}
This description is tailored for schemes that perform calculations over a single finite field, as is the case of the Oil--Vinegar family. However, the bipolar construction can be extended to work differently from the main idea given. We invite the reader to consult~\cite{} for a discussion of schemes that do not employ the bipolar construction at all, or that use multiple finite fields in their construction.



The bipolar construction creates a situation in which its derived schemes
are not exclusively based on the polynomial system solving
problem. Instead, it is proven~\cite{} that there exist many private keys that
compose a single public key. Thus, a scheme using the bipolar construction is
also based on what is called the extended isomorphism of polynomials (EIP)
problem. However, there is little information on the difficulty of EIP,
preventing complete security proofs for most multivariate cryptosystems. %mqdss
