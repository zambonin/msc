...
by the Q-Q plot, which in this case is more commonly referred to as a normal probability plot. It is built by pairing the normalized counts with quantiles from the standard normal distribution. Such quantiles are chosen to approximate normal order statistics~\cite{} and generated as follows. 

Let $r \in \mathbb{N}$ be a precision parameter. A number $r \times q$ of samples are taken from a standard normal distribution and organized into $q$-uples $K_{0}, \dots, K_{r}$.
Given a number $r$ of $q$-uple samples from a standard normal distribution, a $q$-uple of approximate normal order statistics is calculated by taking the mean of every $k$-th smallest value 

created by pairing the normalized counts with approximate normal order statistics, where the points are sufficiently close
