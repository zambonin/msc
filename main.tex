...
\begin{definition}\label{def:matquad}
  ...
  \begin{align}
    ...
    \end{bmatrix}
  \end{align}
  such that $p^{(i)} = \mathbf{x} \cdot \psi(p^{(i)}) \cdot \mathbf{x}^{\textsf{T}}$ for $\mathbf{x} = (x_{1}, \dots, x_{n}, 1)$.
\end{definition}
...
\begin{theorem}[{\cite[Theorem 2]{Beullens:201706}}]
  Given a private key $(\mathcal{C}, \mathcal{L}_{2}) \in \mathsf{OV}(v, o) \times \mathsf{GL}(\mathbb{F}^{n})$ and $\omega \in \mathsf{GL}(\mathbb{F}^{n})$, let the matrix representation of $\mathcal{L}_{2}$ be of the form
  \begin{align}
    \begin{bmatrix}
    ...
    \end{bmatrix}.
  \end{align}
  If $\omega$ has a matrix representation of the form
  \begin{align}
    \begin{bmatrix}
    ...
    \end{bmatrix},
  \end{align}
  with $\mathcal{L}_{2}^{(1)} \in \mathsf{GL}_{v}(\mathbb{F})$, then the matrix representation of $\omega^{-1} \circ \mathcal{L}_{2}$ is of the form
  \begin{align}
    \begin{bmatrix}
...
Then, as long as $\mathcal{L}_{2}^{(1)}$ is invertible, there exists an equivalent key for UOV. This happens with overwhelming probability as per Subsection~\ref{subsec:invert}. Hence, one may simply choose $\widetilde{\mathcal{L}}_{2} \in \mathsf{GL}(\mathbb{F}^{n})$ with its matrix representation of the form
\begin{align}
  \begin{bmatrix}
...
\begin{align}
  \begin{split}
    ...
    \begin{dcases}
      ...
      \end{bmatrix} & \text{for } p^{(1)}, \dots, p^{(o_{1})}, \\
      \begin{bmatrix}
      ...
      \end{bmatrix} & \text{for } p^{(o_{1} + 1)}, \dots, p^{(o_{2})},
    \end{dcases} \\
    1 \leq i \leq m.
  \end{split}
\end{align}
...
\begin{theorem}[{\cite[Theorem 4.17]{Wolf:201104}}]
  Without loss of generality, consider a private key $(\mathcal{L}_{1}, \mathcal{C}, \mathcal{L}_{2}) \in \mathsf{GL}(\mathbb{F}^{m}) \times \mathsf{RB}(v_{1}, o_{1}, \dots, o_{u}) \times \mathsf{GL}(\mathbb{F}^{m})$ with $u = 2$. Given $\tau \in \mathsf{GL}(\mathbb{F}^{m})$ with a matrix representation of the form
  \begin{align}
    \begin{bmatrix}
    ...
    \end{bmatrix},
  \end{align}
  and $\omega \in \mathsf{GL}(\mathbb{F}^{n})$ with a matrix representation of the form
  \begin{align}
    \begin{bmatrix}
    ...
    \end{bmatrix}
  \end{align}
  with $\omega^{(1)} \in \mathsf{GL}_{v_{1}}(\mathbb{F}),\; \tau^{(1)}, \omega^{(3)} \in \mathsf{GL}_{o_{1}}(\mathbb{F})$ and $\tau^{(3)}, \omega^{(6)} \in \mathsf{GL}_{o_{2}}(\mathbb{F})$, then $(\mathcal{L}_{1} \circ \tau^{-1}, \tau \circ \mathcal{C} \circ \omega, \omega^{-1} \circ \mathcal{L}_{2})$ is an equivalent private key.
\end{theorem}

Analogously to the case of UOV, $\tau^{-1}$ and $\omega^{-1}$ maintain their forms. The redundancy is removed by a similar rationale, observing the shape of the lateral maps in the equivalent key.
