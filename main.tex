...
\begin{lemma}[{\cite[Lemma~4]{Kipnis:199808}}]
  Given a polynomial $f^{(i)} \in \textsf{OVH}(v, o)$, let $\phi^{(i)} : \mathbb{F}^{n} \to \mathbb{F}^{n}$ be the linear transformation associated to the matrix $\widetilde{M}(f^{(i)})$, and respectively
\end{lemma}
