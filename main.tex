...
Particularly, Galois fields or \emph{finite fields} are at the heart of various branches of mathematics and computer science, and especially cryptography. They are denoted as $\mathbb{F}_{q^{n}}$, with $q$ a prime number as the characteristic of the field, and $q$ and $n$ omitted for brevity if appropriate.
...
\begin{definition}
  Given $k \in \mathbb{N}$, a set of \emph{coefficients} $A$ with $a_{0}, \dots, a_{k} \in A$, and an \emph{indeterminate} $x$, a \emph{univariate polynomial} is an expression of the form
  \[
    \sum_{i = 0}^{k} a_{i} x^{i} 
...
% TODO univariate formula
Polynomials of degree zero, one, two and three are respectively called \emph{constant}, \emph{linear}, \emph{quadratic} and \emph{cubic}. We note that addition and subtraction between polynomials is done by simply operating coefficients of monomials with the same product of variables, that is, $\sum_{i}^{k} a_{i} x^{i} \pm \sum_{i}^{k} b_{i} x^{i} = \sum_{i}^{k} (a_{i} \pm b_{i}) x^{i}$. Multiplication is calculated through repeated applications of the distributive law. Division is calculated with the Euclidean algorithm, considering $x^{0} < x^{1} < \cdots < x^{k}$ as an intuitive ordering to identify divisors and dividends. This allows us to define the following structure.
...
\begin{definition}
  Given $p_{1}, p_{2}, p_{3} \in \mathsf{R}[\mathbf{x}]$ with $\deg(p_{1}) > 0$, if $p_{1} = p_{2} \ast p_{3}$ implies that either $\deg(p_{2}) = 0$ or $\deg(p_{3}) = 0$, then $p_{1}$ is an \emph{irreducible polynomial over $\mathsf{R}[\mathbf{x}]$}.
\end{definition}
...
\begin{definition}
  Given $p \in \mathbb{F}_{q}[x]$ irreducible with $\deg(p) = \ell$, then the finite field $\mathbb{F}_{q^{\ell}}$ is the set of polynomials with coefficients in $\mathbb{F}_{q}$ and $\deg(\cdot) < \ell$ carrying the usual polynomial arithmetic operations modulo $p$.
\end{definition}
...
\begin{definition}\label{def:polymap}
  Given $n, m \in \mathbb{N}$, a polynomial ring $\mathsf{F} = \mathbb{F}[x_{1}, \dots, x_{n}]$ and $p^{(1)}, \dots, p^{(m)} \in \mathsf{F}$, a \emph{polynomial map} is a function $j : \mathbb{F}^{n} \to \mathbb{F}^{m}$ defined by an $m$-tuple of polynomial functions
  \begin{align}
    j(x_{1}, \dots, x_{n}) = (p^{(1)}(x_{1}, \dots, x_{n}), \dots, p^{(m)}(x_{1}, \dots, x_{n})).
  \end{align}
\end{definition}
...
\begin{definition}
  Given a vector space $(V, \bm{+}, \bm{\times})$ over $\mathbb{F}$, $\mathbf{v_{1}}, \dots, \mathbf{v_{n}} \in V$ and $f_{1}, \dots, f_{n} \in \mathbb{F}$, if the unique solution for the linear combination $f_{1} \bm{\times} \mathbf{v_{1}} \bm{+} \cdots \bm{+} f_{n} \bm{\times} \mathbf{v_{n}} = 0$ is $f_{1} = \cdots = f_{n} = 0$, then the vectors $\mathbf{v_{1}}, \dots, \mathbf{v_{n}}$ are \emph{linearly independent}.
\end{definition}

\begin{definition}
  Given a vector space $\mathbf{V}$ and $B = \{\mathbf{v_{1}}, \dots, \mathbf{v_{n}}\} \subseteq V$, the set of all linear combinations $S = \{ f_{1} \bm{\times} \mathbf{v_{1}} \bm{+} \cdots \bm{+} f_{n} \bm{\times} \mathbf{v_{n}} \mid f_{1}, \dots, f_{n} \in \mathbb{F} \}$ is the \emph{linear span of $B$}. If the elements of $B$ are linearly independent and $S = V$, then $B$ is a \emph{basis of the vector space $\mathbf{V}$}.
\end{definition}
...
\begin{definition}
  Given two vector spaces $\mathbf{V} = (V, \bm{+}, \bm{\times})$ and $\mathbf{V'} = (V', \bm{+'}, \bm{\times'})$ over the same field $\mathbb{F}$, any vectors $\mathbf{v_{1}}, \mathbf{v_{2}} \in V$ and any scalar $f \in \mathbb{F}$, a function $\phi : \mathbf{V} \to \mathbf{V'}$ is a \emph{linear transformation} if it satisfies the following conditions:
  
  \begin{enumerate}
    \item The \emph{addition operation is preserved}, that is, $\phi(\mathbf{v_{1}} \bm{+} \mathbf{v_{2}}) = \phi(\mathbf{v_{1}}) \bm{+'} \phi(\mathbf{v_{2}})$.
    \item The \emph{scalar multiplication operation is preserved}, that is, $\phi(f \bm{\times} \mathbf{v_{1}}) = f \bm{\times'} \phi(\mathbf{v_{1}})$.
  \end{enumerate} 
\end{definition}
...
Multivariate cryptography is based on public-key cryptography. Within this system, an entity holds a key pair, composed of a private key and a public key, hereafter referred to as $\mathbf{sk}$ and $\mathbf{pk}$ respectively. Key pairs are mathematically bound to a computationally hard to solve problem, which is the basis for the security of the resulting cryptosystem. This allows for several frameworks that may ensure distinct properties. We focus on a specific application of public-key cryptography.
...
\begin{definition}\label{def:matquad}
  Given a polynomial system $\mathcal{P} \in \textsf{MQ}(n, m)$, the \emph{matrix representation of a polynomial $p^{(i)} \in \mathcal{P}, 1 \leq i \leq m$} is given by
  \begin{align}
    \overline{M}(p^{(i)}) = 
    \begin{bmatrix}
...
\begin{definition}\label{def:mathom}
  Given a polynomial system $\mathcal{P} \in \textsf{MHQ}(n, m)$, the \emph{matrix representation of a homogeneous quadratic polynomial $p^{(i)} \in \mathcal{P}, 1 \leq i \leq m$} is given by
  \begin{align}
    M(p^{(i)}) = 
    \begin{bmatrix}
...
\begin{definition}
  Given a homogeneous quadratic Oil--Vinegar polynomial $p^{(i)} \in \mathsf{OVH}(v, o)$, its matrix representation is
  \begin{align}
    \widetilde{M}(p^{(i)}) =
    \begin{bmatrix}
...
\begin{align}
  \begin{split}
    ...
    \begin{dcases}
      ...
      \end{bmatrix} & \text{for } p^{(1)}, \dots, p^{(o_{1})} \in \mathcal{C}, \\
      \begin{bmatrix}
      ...
      \end{bmatrix} & \text{for } p^{(o_{1} + 1)}, \dots, p^{(o_{2})} \in \mathcal{C},
    \end{dcases} \\
    1 \leq i \leq m.
  \end{split}
\end{align}
...
The remarks about UOV regarding the size of parameters, chances of failure in the inversion process and lack of security weaknesses are maintained in the case of Rainbow. An instance of Rainbow is denoted by $\mathsf{Rainbow}(\mathbb{F}_{q}, v_{1}, o_{1}, \dots, o_{u})$, with usually $u = 2$ for efficiency reasons. We note that when $u = 1$, we get the UOV scheme.
...
$\overline{\mathcal{C}}$ map linear in $v_{1}$, thus reducing storage requirements. This
...
$\mathcal{C}$, giving $\overline{\mathcal{C}}$. According to
...
create a different $\overline{\mathcal{C}}$. The private key of Rainbow-$\eta_{1}$ is
$(\mathbf{S}, \mathcal{L}_{1}, \overline{\mathcal{C}}, \mathcal{L}_{2})$ and the public key is
$\mathcal{P}$.
...
$\mathcal{C'}$. By the relations above, one is able to reconstruct
$\mathcal{C}$ with no additional mechanisms if the central map inversion
algorithm fails. The private key of Rainbow-$\eta_{2}$ is
$(\mathcal{L}_{1}, \overline{\mathcal{C}}, \mathcal{L}_{2})$ and the public key is $\mathcal{P}$.
...
a new $\widetilde{V}_{1}$ is chosen and $\overline{\mathcal{C}}$ regenerated, according
...
recalculation of $\overline{\mathcal{C}}$ in the case that $\widetilde{V}_{1}$ is not
...
for this scheme is $(\mathcal{L}_{1}, \overline{\mathcal{C}}, \mathcal{L}_{2}, w)$, with the
...
the rare case that the $x = \overline{\mathcal{C}}(y)$ preimage calculation does not
...
process, such that $\widetilde{V}_{1}$, and consequently $\overline{\mathcal{C}}$, are
...
particularly ones that take into account multiple signatures, due to our fixing
