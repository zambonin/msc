...
As mentioned previously, if a linear transformation is of the form $\varphi : \mathbf{V} \to \mathbf{V}$, it is an endomorphism. If $\dim V = \dim W$, then $\varphi$ may be invertible. In other words, given the trivial endomorphism $\textsf{id} : \mathbf{V} \to \mathbf{V}$ such that $\textsf{id}(\mathbf{v_{1}}) = \mathbf{v_{1}}$, there may exist a function $\varphi^{-1} : \mathb{}$
