...
We note that there is no need to provide a reconstruction procedure of $\overline{\mathcal{C}}$ as in the case of Rainbow-$\eta_{1}$, which needs a seed, and Rainbow-$\eta_{2}$, that features non-trivial mathematical relations. The private key size of Rainbow-$\eta_{4}$ is similar to Proposition~\ref{prop:eta-key-size}, but every $v_{1}$ quantity is substituted by $v_{1} - \text{wt}(\mathbf{v})$, increasing the key size depending on the Hamming weight of the vinegar threshold, which also needs to be stored. We now expand on the security of our new proposal.
