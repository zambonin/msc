...
The process for Rainbow is analogous, with a small difference due to the existence of two lateral maps. Since $\mathcal{L}_{1}$ simply permutes coefficients maintaining the structure of $\mathcal{C}$, we let $\mathcal{Q} = \mathcal{C} \circ \mathcal{L}_{2}$, and consequently $\mathcal{P} = \mathcal{L}_{1} \circ \mathcal{Q}$. The relationship between $\mathcal{Q}$ and $\mathcal{C}$ is exactly as described above. Let $q_{ij}^{k}$ be the coefficient associated with the monomial $x_{i} x_{j}$ in the $k$-th component of $\mathcal{Q}$. It is equal to the right-hand side of Equation~\ref{}.

\begin{align}
  \vu_{ij}^{(k)} = \sum_{l = v_{1} + 1}^{n} s_{k - v{1}, l - v_{1}} q_{ij}^{(l)}
\end{align}
