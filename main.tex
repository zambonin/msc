...
The bipolar construction creates a situation in which its derived schemes are not exclusively based on the polynomial system solving problem. Instead, it is proven that there exist many private keys that compose a single public key~\cite{Wolf:201104}. The theory of \empn{equivalent keys} has been used to attack several schemes, as seen in Section~\ref{sec:related}. Thus, a scheme using the bipolar construction is also based on some variant of the \emph{isomorphism of polynomials problem}. 
