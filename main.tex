...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item There exists a uniquely determined element $g_{1}^{-1} \in G$, called \emph{the inverse of $g_{1}$}, such that $g_{1} \star g_{1}^{-1} = g_{1}^{-1} \star g_{1} = e$, and indeed $(g_{1}^{-1})^{-1} = g_{1}$.
  \end{enumerate}
\end{definition}
...
Now, we extend the definition of a group to form another structure carrying two binary operations, usually called addition and multiplication.
...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item The \emph{distributive laws} hold, that is, $\times$ distributes over $+$ and $+$ distributes over $\times$. In other words, $(r_{1} + r_{2}) \times r_{3} = (r_{1} \times r_{3}) + (r_{2} \times r_{3})$ and $(r_{1} \times r_{2}) + r_{3} = (r_{1} + r_{3}) \times (r_{2} + r_{3})$.
  \end{enumerate}
\end{definition}
