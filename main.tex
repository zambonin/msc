...
If it is clear from the context, we hereafter refer to a group simply by $G$ and omit the associated binary operation $\star$. Other useful facts about groups are given in~\cite[Prop. 1]{}.

\begin{definition}
  Given a group $(G, \star)$, if $G$ is finite, then the group is also called \emph{finite}. A group is never ``empty'', due to the existence of the identity~\ref{def:group}
\end{definition}
