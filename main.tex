...
\begin{definition}\label{def:mathom}
  Given a polynomial system $\mathcal{P} \in \textsf{MQ}(m, n)$, the \emph{matrix representation of a homogeneous quadratic polynomial $p^{(i)} \in \mathcal{P}, 1 \leq i \leq m$} is given by
  \begin{align}
    M(p^{(i)}) = 
    \begin{bmatrix}
...
\begin{align}
  \begin{split}
    \mathbf{sk} &= (\mathcal{L}_{1}, \mathcal{C}, \mathcal{L}_{2})
      \:\quad \in \mathsf{GL}_{n}(\mathbb{F}) \times \mathsf{MQ}(n, m) \times \mathsf{GL}_{m}(\mathbb{F}), \\
    \mathbf{pk} &= \mathcal{L}_{1} \circ \mathcal{C} \circ \mathcal{L}_{2} 
      \quad \in \mathsf{MQ}(n, m).
  \end{split}
\end{align}
