...
Thus, the only remaining task is to build a basis $B$ of $\widetilde{\mathcal{O}}$, which has dimension $m$. We leave it as an open question to calculate a lower bound for the size of $\rho$ in order to obtain $\widetilde{\rho} = \{ \chi_{1}, \dots, \chi_{m} \}$. We give

As seen in Section~\ref{sec:stats}, signatures are statistically similar to random elements of the underlying vector space. Thus, it is expected with high probability that all $\chi_{i}$ are linearly independent.
