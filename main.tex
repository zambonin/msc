...
\begin{align}
  ...
  \end{bmatrix}.
\end{align}
The associated linear transformation $\widetilde{\mathcal{L}}_{2}$ and proceed to recover an equivalent key as per Equation~\ref{eq:uov-attack-eq}. Finally, an attacker can use $\mathsf{sk'}$ and forge signature for any message. In fact, if the method of Rainbow-$\eta$ is not applied to the equivalent key, the forged signatures do not leak any information. 
