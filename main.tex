...
We note that there is no need to provide a reconstruction procedure of $\overbar{\mathcal{C}}$ as in the case of Rainbow-$\eta_{1}$, with a seed, and Rainbow-$\eta_{2}$, with mathematical relations. Further, the private key size of Rainbow-$\eta_{4}$ is similar to Proposition~\ref{prop:eta-key-size}, with the added size of $\mathbf$ but the size of $\widetilde{V}_{1}$ is now $v_{1} - \text{wt}()$
