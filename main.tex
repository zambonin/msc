...
\begin{definition}
  Given a set $G$ and any $g_{1}, g_{2} \in G$, a \emph{binary operation $\star$ on a set $G$} is a function $\star : G \times G \to G$, where we denote $\star(g_{1}, g_{2})$ as $g_{1} \star g_{2}$. A binary operation $\star$ on a set $G$ may also satisfy the following conditions:
  
  \begin{itemize}
    \item It is said to be \emph{associative} if, for any $g_{1}, g_{2}, g_{3} \in G$, we have that $g_{1} \star (g_{2} \star g_{3}) = (g_{1} \star g_{2}) \star g_{3}$.
    \item It is said to be \emph{commutative} if all elements of $G$ commute, that is, for any $g_{1}, g_{2} \in G$, we have that $g_{1} \star g_{2} = g_{2} \star g_{1}$.
    \item It is said to be \emph{closed under $G$} if, for any $g_{1}, g_{2} \in G$, we have that $g_{1} \star g_{2} \in G$.
  \end{itemize}
\end{definition}

\begin{definition}
  Given a set $G$ and a binary operation $\star$ on $G$, a \emph{group} is an ordered pair $(G, \star)$ that satisfies the following axioms:
  
  \begin{enumerate}
    \item The binary operation $\star$ is \emph{closed under $G$}.
    \item The binary operation $\star$ on $G$ is \emph{associative}.
    \item For any $g_{1} \in G$, there exists a unique element $e \in G$, called \emph{the identity of $G$}, such that $g_{1} \star e = e \star g_{1} = g_{1}$.
    \item For any $g_{1} \in G$, there exists a uniquely determined element $g_{1}^{-1} \in G$, called \emph{the inverse of $g_{1}$}, such that $g_{1} \star g_{1}^{-1} = g_{1}^{-1} \star g_{1} = e$, and indeed $(g_{1}^{-1})^{-1} = g_{1}$.
  \end{enumerate}
\end{definition}

\begin{definition}
  Given a group $(G, \star)$, if $G$ is a finite set, then the group is also called \emph{finite}. A group is never ``empty'', due to the existence of the identity.
\end{definition}
...
If it is clear from the context, we hereafter refer to a group simply by $G$ and omit the associated binary operation $\star$. Other useful facts about groups are given in~\cite[Prop. 1]{}. Groups carrying the usual addition or multiplication operations are called \emph{additive} and \emph{multiplicative} groups, respectively. Common examples of groups are the additive group under $\mathbb{Z}$, that is, $(\mathbb{Z}, +)$, and the multiplicative group of integers modulo $n$ denoted as $(\mathbb{Z}/n\mathbb{Z})^{\times}$.

%  A vector space of dimension $n$ over the finite field $\mathbb{F}_{q}$ of order $q$ is denoted by $\mathbb{F}_{q}^{n}$, with $q$ and $n$ omitted for brevity if appropriate.
