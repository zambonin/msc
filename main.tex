...
The trivial, or zero ring, is defined as $(\{0\}, +, \times)$ with usual addition and multiplication operations.
...
\begin{theorem}
  The order of a finite field is always of the form $q^{n}$ with $q$ prime and $n \in \mathbb{N}$  
\end{theorem}

We are especially interested in \emph{finite} fields, or Galois fields, usually represented by $\mathbb{F}$. A finite field always has a prime characteristic~\cite{}, denoted by $q$, and thus its order may only be a prime or a prime power $q^{n}, n > 1$.
