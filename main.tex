...
\end{align}
Focusing on the inner summation, we can represent it as
\begin{align}
  \alpha_{ij}^{(k)} \sum_{r = 1}^{n} \sum_{s = 1}^{n} b_{rs}^{ij} x_{r} x_{s}, 
  \quad \text{where } b_{rs}^{ij} =
  \begin{dcases}
    d_{ir} \cdot d_{jr}                         & \text{if } r = s, \\
    d_{ir} \cdot d_{js} + d_{is} \cdot d_{jr}   & \text{otherwise}.
  \end{dcases}
\end{align}
The public polynomial of Equation~\ref{} is then rewritten as
\begin{align}
  \widetilde{h}^{(k)} = \sum_{i = 1}^{n} \sum_{j = i}^{n} \left( \alpha_{ij}^{(k)} \sum_{r = 1}^{n} \sum_{s = 1}^{n} b_{rs}^{ij} x_{r} x_{s} \right).
\end{align}
The coefficient associated with the monomial $x_{i} x_{j}$ of the public key is thus exactly
\begin{align}
  \nu_{ij}^{(k)} = \sum_{i = 1}^{n} \sum_{j = i}^{n} \alpha_{ij}^{(k)} b_{rs}^{ij}.
\end{align}
By fixing a specific $\mathcal{L}_{2}$, Equation~\ref{} becomes a linear relation between the coefficients of the private and public keys. This allows for the reconstruction of the quadratic part of $\mathcal{C}$ from $\mathcal{P}$ and $\mathcal{L}_{2}$. Thus, if the scheme uses homogeneous quadratic polynomials, the entire central map can be reconstructed.

The process for Rainbow is analogous, with a small difference due to the existence of two lateral maps. Since $\mathcal{L}_{1}$ simply permutes coefficients maintaining the structure of $\mathcal{C}$, we let $\mathcal{Q} = \mathcal{C} \circ \mathcal{L}_{2}$, and consequently $\mathcal{P} = \mathcal{L}_{1} \circ \mathcal{Q}$. The relationship between $\mathcal{Q}$ and $\mathcal{C}$ is exactly as described above. Let $q_{ij}^{k}$ be the coefficient associated with the monomial $x_{i} x_{j}$ in the $k$-th component of $\mathcal{Q}$. Then,
\begin{align}
  q_{ij}^{(k)} = \sum_{i = 1}^{n} \sum_{j = i}^{n} \alpha_{ij}^{(k)} b_{rs}^{ij},
\end{align}
and by letting the elements of $\psi(\mathcal{L}_{1})$ be denoted as $e_{ij}, 1 \leq i, j \leq n$, then
\begin{align}
  \nu_{ij}^{(k)} = \sum_{l = v_{1} + 1}^{n} q_{ij}^{(l)} e_{k - v{1}, l - v_{1}}.
\end{align}
