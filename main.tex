...
\begin{definition}
  Given a multivariate cryptosystem based on the bipolar construction and the key pair $(\mathbf{sk}, \mathbf{pk})$ from its key generation algorithm, finding a decomposition $\mathbf{pk} = \widetilde{\mathcal{L}_{1}} \circ \widetilde{\mathcal{C}} \circ \widetilde{\mathcal{L}_{2}}$ with the same structure as the corresponding parts of $\mathbf{sk}$ is called the \emph{extended isomorphism of polynomials problem}.
\end{definition}
