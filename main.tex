...
Interestingly, this attack can be used to recover a private key of a legitimate signer. It must compute in advance several linear independent preimages of $\mathcal{C}$ with the same vinegar variables. In the event that the private key is lost, the signer would only need to compute an equivalent private key by fetching i

a legitimate signer can compute several linear independent preimages of random messages with the same vinegar variables, and in the event that 
