...
It is argued in~\cite[Lemmata 3.2, 3.10, 3.19]{Thomae:201306} that some IP problems modelled to represent Oil--Vinegar schemes are no harder to solve if $\mathcal{L}_{1}$ and $\mathcal{L}_{2}$ are linear instead of affine transformations. As a consequence, the polynomials in the central map must be homogeneous quadratic. Our description is based on this technical results. There is also independent evidence that using linear transformations and quadratic maps is safe both in UOV~\cite[Section 5]{Kipnis:199808} and Rainbow~\cite[Section 3.1]{Ding:201901}, but that may not be the case with other schemes built upon the bipolar construction.
