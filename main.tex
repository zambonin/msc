...
We remark that the symmetric matrix of Definition~\ref{def:mathom} has a diagonal of zeroes if the underlying field characteristic is even. Now, we recall the definition of two essential building blocks for a multivariate signature scheme, with definitions takes from~\cite{} and~\cite{}.
...
\begin{definition}
  ...
  The set $X$ is usually all binary words $\{0, 1\}^{*}$, while $Y$ is usually $\mathbb{F}_{2}^{k}$ for some small $k \in \mathbb{N}$.
\end{definition}

The cryptographic hash function is the usual method of transforming a variable-length message into a fixed-length \emph{digest}. Such a well-designed function outputs an unpredictable string with no apparent connection to the input, which change completely even with the smallest difference in the input.
