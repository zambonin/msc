...
\begin{definition}
  The vector subspace of $\mathbb{F}^{n}$ that contains all vectors with the first $v$ coordinates equal to zero, that is, $\mathcal{O} = \{ (0, \dots 0, x_{v + 1}, \dots, x_{n}) \in \mathbb{F}^{n} \}$, is the \emph{oil subspace}.
\end{definition}
