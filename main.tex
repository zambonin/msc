...
Nonetheless, we identify that a sequence of signatures created with the same vinegar variables can be used to mount a key recovery attack. A malicious entity is thus able to create signature forgeries in polynomial time, similar to the UOV attack. The key observation is that signatures produced with Rainbow-$\eta$ are vectors of $\widetilde{\mathcal{O}}$. Consequently, an equivalent linear transformation $\widetilde{\mathcal{L}}_{2}$ can be recovered and composed with $\mathcal{P}$ to yield an equivalent private key.
...
Let this set be denoted as $\rho = (\sigma_{1}, \dots, \sigma_{m + 1})$. Any preimage of $\mathcal{C}$ always starts with $\widetilde{V}_{1} = (x_{1}, \dots, x_{v})$.

The first $v$ variables of any $\sigma_{i}$ under the transformation $\mathcal{L}_{2}^{-1}$ are equal to the chosen sequence of vinegar variables $\widetilde{V}_{1} = (x_{1}, \dots, x_{v})$. In other words, $\mathcal{L}_{2}^{-1}(\sigma_{i}) = (x_{1}, \dots, x_{v}, \ast, \dots, \ast)$. Since this is the case for any $\sigma_{i}$, we have that
\begin{align}
  \mathcal{L}_{2}^{-1}(\sigma_{i} - \sigma_{i + 1}) = (0, \dots, 0, \ast, \dots, \ast).
\end{align}
In fact, any subtraction between two signatures in $\rho$ yields vectors starting with $v$ zeroes. Let us call such a vector $\chi_{i}$, and evidently $\chi_{i} \in \mathcal{L}_{2}^{-1}(\mathcal{O})$. Since the attacker's goal is to build a basis of this set, then $m$ linearly independent $\chi_{i}$ must be collected such that the basis can be constructed. Let us call this set $\widetilde{\rho}$. We note that at least $m + 1$ signatures need to be collected to perform $m$ subtractions.
