...
\begin{definition}\label{def:poly-sys-quad}
...
\end{align}
field elements. We note that if the Oil--Vinegar polynomials are homogeneous quadratic, then the private key size decreases in $o \cdot (v + o + 1)$ elements, and the public key size decreases in $o \cdot (n + 1)$ elements.
...
\end{align}
field elements. If polynomials are homogeneous quadratic, then the private key size decreases in $\sum_{k = 1}^{u} o_{k} \cdot (v_{k} + o_{k} + 1)$ elements, and the public key size decreases in $m \cdot (n + 1)$ elements.
...
\begin{theorem}[{\cite[Theorem 3.6]{Petzoldt:201307}}]\label{thm:eqrb}
...
Consider the key pair of a UOV signature scheme as described in Equations~\ref{eq:skov} and~\ref{eq:pkov}. It is known that quadratic terms do not mix with linear and constant terms in the case of Oil--Vinegar signature schemes, and thus we refer only to the quadratic part of any polynomial systems in our description. Let the quadratic part of the $k$-th polynomial $c^{(k)} \in \mathcal{C},\; 1 \leq k \leq m$, represented by Equation~\ref{eq:ov}, be denoted as
\begin{align}
...
\begin{align}\label{eq:pkquad}
  \widetilde{h}^{(k)} = \sum_{i = 1}^{n} \sum_{j = i}^{n} \nu_{ij}^{(k)} x_{i} x_{j}.
\end{align}
Let the elements of $\psi(\mathcal{L}_{2})$ be denoted as $d_{ij}, 1 \leq i, j \leq n$. We recall that the function composition $\mathcal{P} = \mathcal{C} \circ \mathcal{L}_{2}$ can be represented as a series of matrix multiplication operations. For instance, Equation~\ref{eq:pkquad} may be rewritten as
\begin{align}\label{eq:pkquadmult}
...
\begin{align}
  ...
  \end{dcases}
\end{align}
allowing the rewrite of Equation~\ref{eq:pkquadmult} as
\begin{align}
  \widetilde{h}^{(k)} = \sum_{i = 1}^{n} \sum_{j = i}^{n} \left( \alpha_{ij}^{(k)} \sum_{r = 1}^{n} \sum_{s = 1}^{n} d_{rs}^{(ij)} x_{r} x_{s} \right).
\end{align}
If we recall Equation~\ref{eq:pkquad}, the coefficient associated with the monomial $x_{i} x_{j}$ of the public key is thus exactly
\begin{align}\label{eq:linear}
  \nu_{ij}^{(k)} = \sum_{i = 1}^{n} \sum_{j = i}^{n} \alpha_{ij}^{(k)} \sum_{r = 1}^{n} \sum_{s = 1}^{n} d_{rs}^{(ij)}.
\end{align}
By fixing a specific $\mathcal{L}_{2}$, Equation~\ref{eq:linear} becomes a linear relation between the coefficients of the private and public keys. This allows for the reconstruction of the quadratic part of $\mathcal{C}$ from $\mathcal{P}$ and $\mathcal{L}_{2}$. Thus, if the scheme uses homogeneous quadratic polynomials, the entire central map can be reconstructed. In the literature, this is depicted by representing the coefficients of maps $\mathcal{C}$ and $\mathcal{P}$ as a matrix with respect to some monomial ordering, and analogously for $d_{rs}^{(ij)}$~\cite[p.~94]{Petzoldt:201307}.

The process for Rainbow is similar, with a small difference due to the presence of two lateral maps in the private key. Equations~\ref{eq:skrb} and~\ref{eq:pkrb} show, respectively, the private and public keys of Rainbow. Since $\mathcal{L}_{1}$ simply permutes coefficients maintaining the structure of $\mathcal{C}$, we let $\mathcal{Q} = \mathcal{C} \circ \mathcal{L}_{2}$, and consequently $\mathcal{P} = \mathcal{L}_{1} \circ \mathcal{Q}$. The relationship between $\mathcal{Q}$ and $\mathcal{C}$ is exactly as described above in the case of UOV. Let $q_{ij}^{(k)}$ be the coefficient associated with the monomial $x_{i} x_{j}$ in the $k$-th component of $\mathcal{Q}$. Then,
\begin{align}
...
\subsection{Application to the EU-CMA variant}\label{subsec:eucma}
...
\begin{proposition}
  ...
  \#\mathsf{sk}^{\eta} = \underbrace{m^{2}}_{\mathcal{L}_{1}}
    + \underbrace{n^{2}}_{\mathcal{L}_{2}} 
    + \underbrace{\widetilde{V}_{1}}_{v_{1}} + \\
...
\end{proposition}
\begin{proof}
  We simply subtract $v_{1}$ terms from every collection of $v_{k}$ monomials in Equation~\ref{eq:priv}, and add $\widetilde{V}_{1}$ such that whole preimages are computed.
\end{proof}
...
The multi-layer approach of Rainbow does not hinder attacks that also work on the UOV signature scheme. This attack was originally created by Kipnis and Shamir~\cite{Kipnis:199808} to break the balanced Oil--Vinegar scheme. The objective of this attack is to obtain an equivalent private key by means of finding the preimage of a specific oil subspace under the map $\mathcal{L}_{2}$. The complexity of the generalized attack for unbalanced schemes~\cite{Kipnis:199904} is $o_{u}^{4} \cdot q^{n - 1 - 2 \cdot o_{u}}$ field multiplications. We discuss the UOV attack in more detail on Chapter~\ref{ch:attack}.
...
We observe the distribution of field elements in vectors after the final function application, that is, $\mathbf{z} = \mathcal{L}_{2}^{-1}(\mathbf{x})$. Consider a $t$-tuple of signatures from an unmodified instance of Rainbow acting as a control group, that is, $Z_{t}' = (\mathbf{z}_{1}, \dots, \mathbf{z}_{t}),\; t \in \mathbb{N}$. Let its ``unpacked notation'' be the $(n \times t)$-tuple $Z_{t} = (z_{1}^{1}, z_{1}^{2}, \dots, z_{1}^{n}, z_{2}^{1}, \dots, z_{t}^{n - 1}, z_{t}^{n})$ of elements in $\mathbb{F}$. When the first $v_{1}$ elements of the vector $\mathbf{x}$ are fixed due to the usage of Rainbow-$\eta$, we instead denote these by $\widetilde{Z}'_{t}$ and $\widetilde{Z}_{t}$. 
...
Let $\mathcal{U}(1, q)$ be the discrete uniform distribution that represents the chance of picking an element from $\mathbb{F}$. We note that if the field characteristic is even, there exists a mapping to the natural numbers that allows this choice to be performed. Thus, we expect that
\begin{align}
...
Our method does not depend on special structures inserted on the private key. Consequently, it can be applied to all known Rainbow-like schemes. We experiment with several sets of parameters and observe the reduction of private key sizes. It is known that there are various limitations for the choice of parameters that lead to secure instances of Rainbow~\cite{Petzoldt:201005}. We implement several known guidelines and confirm that our proposal does indeed work for a large range of parameters. 

We only show results for known secure parameter sets to prevent accidental endorsement of untested, and possibly insecure, instances. Furthermore, we remark that comparisons are made considering the usage of homogeneous quadratic polynomials and simplified linear transformations according to Theorem~\ref{thm:eqrb}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{*{2}{l}*{5}{r}}
    ...
    I-a    & $(\mathbb{F}_{ 16}, 32, 32, 32)$  &   96 &   64 &    92928 &   26896 &  $-71.06\%$ \\
    I-b    & $(\mathbb{F}_{ 31}, 36, 28, 28)$  &   92 &   56 &   105916 &   24628 &  $-76.75\%$ \\
    I-c    & $(\mathbb{F}_{256}, 40, 24, 24)$  &   88 &   48 &   132576 &   24136 &  $-81.79\%$ \\
    III-b  & $(\mathbb{F}_{ 31}, 64, 32, 48)$  &  144 &   80 &   389975 &   71552 &  $-81.65\%$ \\
    III-c  & $(\mathbb{F}_{256}, 68, 36, 36)$  &  140 &   72 &   511416 &   78188 &  $-84.71\%$ \\
    IV-a   & $(\mathbb{F}_{ 16}, 56, 48, 48)$  &  152 &   96 &   358656 &   88540 &  $-75.31\%$ \\
    V-c    & $(\mathbb{F}_{256}, 92, 48, 48)$  &  188 &   96 &  1227072 &  180572 &  $-85.28\%$ \\
    VI-a   & $(\mathbb{F}_{ 16}, 76, 64, 64)$  &  204 &  128 &   860800 &  206630 &  $-76.00\%$ \\
    VI-b   & $(\mathbb{F}_{ 31}, 84, 56, 56)$  &  196 &  112 &   980524 &  187172 &  $-80.91\%$ \\
    P-080  & $(\mathbb{F}_{256}, 17, 13, 13)$  &   43 &   26 &    16757 &    4177 &  $-75.07\%$ \\
    P-100  & $(\mathbb{F}_{256}, 26, 16, 17)$  &   59 &   33 &    41163 &    8364 &  $-79.68\%$ \\
    P-128  & $(\mathbb{F}_{256}, 36, 21, 22)$  &   79 &   43 &    96288 &   17754 &  $-81.56\%$ \\
    P-192  & $(\mathbb{F}_{256}, 63, 46, 22)$  &  131 &   68 &   416998 &   52417 &  $-87.43\%$ \\
    P-256  & $(\mathbb{F}_{256}, 85, 63, 30)$  &  178 &   93 &  1043295 &  128950 &  $-87.64\%$ \\
    \bottomrule
  \end{tabular}
\end{table}

We show results for the application of our method in Table~\ref{tab:1}, considering the following Rainbow instances. Conservative choices were made by the Rainbow submission authors~\cite{Ding:201712} to fit security categories as requested by NIST\@. We apply our method to these recent proposals, and additionally choose parameters from Petzoldt~\cite[Tables~6.12,~9.8]{Petzoldt:201307} for further comparison. The latter are named P-$\ell$, where $\ell$ is the classical security level in bits. 
...
\begin{table}[htbp]
  ...
  \begin{tabular}{*{2}{l}*{4}{r}}
    ...
    \multirow{3}{*}{P-080}  & Classic  &  \multirow{3}{*}{  16757} &  \multirow{3}{*}{   4177} &    24596 &  $-30.42\%$ \\
                            & Cyclic   &                           &                           &     9474 &  $-66.99\%$ \\
                            & LRS2     &                           &                           &     8645 &  $-68.99\%$ \\
    \multirow{3}{*}{P-100}  & Classic  &  \multirow{3}{*}{  41163} &  \multirow{3}{*}{   8364} &    58410 &  $-32.94\%$ \\
                            & Cyclic   &                           &                           &    20266 &  $-71.25\%$ \\
                            & LRS2     &                           &                           &    18682 &  $-72.84\%$ \\
    \multirow{3}{*}{P-128}  & Classic  &  \multirow{3}{*}{  96288} &  \multirow{3}{*}{  17754} &   135880 &  $-33.83\%$ \\
                            & Cyclic   &                           &                           &    44971 &  $-72.98\%$ \\
                            & LRS2     &                           &                           &    42107 &  $-74.22\%$ \\
    \multirow{2}{*}{I-a  }  & Classic  &  \multirow{2}{*}{  92928} &  \multirow{2}{*}{  26896} &   148992 &  $-27.29\%$ \\
                            & nCyclic  &                           &                           &    58112 &  $-64.86\%$ \\
    \multirow{2}{*}{III-c}  & Classic  &  \multirow{2}{*}{ 511416} &  \multirow{2}{*}{  78188} &   710640 &  $-35.45\%$ \\
                            & nCyclic  &                           &                           &   206712 &  $-76.69\%$ \\
    \multirow{2}{*}{V-c  }  & Classic  &  \multirow{2}{*}{1227072} &  \multirow{2}{*}{ 180572} &  1705536 &  $-35.68\%$ \\
                            & nCyclic  &                           &                           &   491904 &  $-77.07\%$ \\
    \bottomrule
  \end{tabular}
\end{table}

We compare key pair sizes when our method is used alongside Rainbow variants that reduce the public key size. Table~\ref{tab:2} presents results for some sets of parameters from Table~\ref{tab:1}. We calculate $\#\mathsf{pk}$ for the variants according to~\cite[Eq.~9.2,~9.4]{Petzoldt:201307} and~\cite{Petzoldt:202004}. As per~\cite[Remark 9.1]{Petzoldt:201307}, we note that $q = 16$ and $q = 31$ are not considered due to a security restriction of RainbowLRS2. The ``nCyclic'' method is a small improvement over CyclicRainbow featured on the second round submission of Rainbow to the NIST standardization process~\cite{Ding:201901}. We obtain positive results, with key pair size reductions of up to factors of 3.

The use of CyclicRainbow or RainbowLRS2 with Rainbow-$\eta_{2}$ is recommended. These variants are based on the linear relations described in Subsection~\ref{sec:eta}, and resulting implementations may be effortlessly modified to use our proposal. Moreover, in the case that higher parameters are needed, \emph{e.g.} a security level of $256$ bits, we note that the key pair is reduced more aggressively. Thus, our results reflect changes over a wide variety of platforms and possible Rainbow deployments that benefit from lower storage requirements.
