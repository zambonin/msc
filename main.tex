...
A trivial example is the identity function $\textsf{id} : \mathbf{V} \to \mathbf{V}$ such that $\textsf{id}(\mathbf{v_{1}}) = \mathbf{v_{1}}$. If additionally $\dim \mathbf{V} = \dim \mathbf{V'}$, there may exist an inverse function $\varphi^{-1} : \mathbf{V'} \to \mathbf{V}$ such that $\varphi \circ \varphi^{-1} = \textsf{id}$. We remark that Definitions~\ref{} and~\ref{} are analogous in the setting of vector spaces. Indeed, there exists an automorphism group $\textsf{Aut}(\mathbf{V})$, which is the set of all invertible linear transformations carrying usual function composition.

This group is more commonly known as the \emph{general linear group} $\mathsf{GL}(\mathbf{V})$. For finite vector spaces of dimension $n$, this group is isomorphic to the general linear group of degree $n$, or $\mathsf{GL}_{n}(\mathbb{F})$. We observe that the notation does not include any reference to vector spaces. This is due to the notable fact that linear transformations may be interpreted as matrices over the base field of the vector space. We expand on this fact below.
