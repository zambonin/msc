...
We note that public keys in multivariate cryptography are polynomial systems with no apparent structure. Thus, if a malicious party can efficiently solve such systems, it is able to perform signature forgery, defeating the security of the scheme. It is thus crucial that solving random instances of polynomial systems is computationally hard, such that a cryptosystem can be based upon this problem. Fortunately, this is the case for several classes of polynomial systems.
...
\begin{theorem}[{~\cite{}}]
  Given $n, m \in \mathbb{N}$, $\mathbf{x} = x_{1}, \dots, x_{n}$, a polynomial ring $\mathbb{F}_{2}[\mathbf{x}]$ and $p^{(1)}, \dots, p^{(m)} \in $, finding $p^{(1)} = 0, \dots, p^{(m)} = 0$ is NP-hard.
\end{theorem}

We start by introducing which polynomials are chosen in this context and how they interact to hide secret information and create a cryptosystem.

Most modern multivariate cryptography schemes are based on multivariate quadratic equations over finite fields, that is, polynomial systems with $\deg(\cdot) = 2$. We discuss this choice in Subsection~\ref{} in details. We expand  Definition~\ref{def:polymap} to its explicit form, taking in account the degree restriction of the polynomials.
