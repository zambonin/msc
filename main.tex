...
\begin{align}
  \mathbf{sk} = (\mathcal{C}, \mathcal{L}_{2})
    \random \mathsf{OV}(v, o) \times \mathsf{GL}_{n}(\mathbb{F}).
\end{align}
...
As mentioned in Section~\ref{sec:mult}, there exists a specific form of sustainer for each scheme. In the case of UOV, these mappings are given in~\cite[Section 4.3]{Wolf:201104}. We follow the notation of~\cite{Petzoldt:201307}.

\begin{theorem}[{\cite[Theorem 4.15]{Wolf:201104}}]
  Given a private key $(\mathcal{C}, \mathcal{L}_{2}) \in \mathsf{OV}(v, o) \times \mathsf{GL}_{n}(\mathbb{F})$ and $\omega \in \mathsf{GL}_{n}(\mathbb{F})$ of the form
  \begin{align}
    \begin{bmatrix}
...
It is a direct consequence of this theorem that there exist at least as many equivalent keys as the possible choices for $\omega$. This redundancy may be cast away by observing the shape of $\omega^{-1} \circ \mathcal{L}_{2}$ and only selecting lateral maps with this structure.

\begin{theorem}[{\cite[Theorem 2]{Beullens:201706}}]
  Given a private key $(\mathcal{C}, \mathcal{L}_{2}) \in \mathsf{OV}(v, o) \times \mathsf{GL}_{n}(\mathbb{F})$ and $\omega \in \mathsf{GL}_{n}(\mathbb{F})$. If
  \begin{align}
    \mathcal{L}_{2} = 
    \begin{bmatrix}
...
Then, as long as $\mathcal{L}_{2}^{(1)}$ is invertible, there exists an equivalent key for UOV. This happens with overwhelming probability as per Subsection~\ref{subsec:invert}. Hence, one may simply choose $\widetilde{\mathcal{L}}_{2} \in \mathsf{GL}_{n}(\mathbb{F})$ of the form
\begin{align}
  \begin{bmatrix}
...
\begin{align}
  \#\mathsf{sk} = \underbrace{m^{2}}_{\mathcal{L}_{1}} 
    ...
    + \underbrace{n^{2}}_{\mathcal{L}_{2}}.
\end{align}
If polynomials are homogeneous quadratic, then the private key size decreases in $\sum_{k = 1}^{u} o_{k} \times (v_{k} + o_{k} + 1)$. The size of a public key is similar to the UOV case, and thus
\begin{align}
