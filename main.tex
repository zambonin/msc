...
\begin{definition}
  Given a private key $(\mathcal{L}_{1}, \mathcal{C}, \mathcal{L}_{2}) \in \mathbb{U}$, the linear transformations $\tau \in \mathsf{GL}_{m}(\mathbb{F}), \omega \in \mathsf{GL}_{n}(\mathbb{F})$ are \emph{sustainers} if
  \begin{align}
    \mathcal{P} = (\mathcal{L}_{1} \circ \tau^{-1}) \circ () \circ ()
  \end{align}
\end{definition}
