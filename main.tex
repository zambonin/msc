...
\begin{definition}
  Given $m = o$, a polynomial map $\mathcal{C} : \mathbb{F}^{n} \to \mathbb{F}^{m}$ consisting of Oil--Vinegar polynomials is an \emph{Oil--Vinegar polynomial map}. The set of all such maps is defined as $\mathsf{OV}(n, m)$.
\end{definition}

In the Oil--Vinegar signature scheme, we have $m = o$, such that after the substitution of vinegar variables, the resulting system is exactly determined, with $m$ equations and $m$ variables.
