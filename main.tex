...
\begin{lemma}[~\cite{}]
  Given a group $\mathsf{G}$, the set of all automorphisms carrying the usual composition binary operation forms a group, denoted $\mathsf{Aut}(\mathsf{G})$.
\end{lemma}
...
Analogously to group- and ring-like structures, there also exist mappings between vector spaces that must ``preserve'' both binary operations. The general form of such functions is presented below.
...
As mentioned previously, if a linear transformation is of the form $\varphi : \mathbf{V} \to \mathbf{V}$, it is an endomorphism. The trivial example is the identity function $\textsf{id} : \mathbf{V} \to \mathbf{V}$ such that $\textsf{id}(\mathbf{v_{1}}) = \mathbf{v_{1}}$. If additionally $\dim \mathbf{V} = \dim \mathbf{V'}$, there may exist a function $\varphi^{-1} : \mathbf{V'} \to \mathbf{V}$ such that $\varphi \circ \varphi^{-1} = \textsf{id}$. In this case, $\varphi$ is invertible and called an isomorphism. An endomorphism $\varphi$ that is also an isomorphism is called an automorphism.
...
% topics for tomorrow: invertible matrix <-> invertible map, general linear group, agl, matrix representations of certain polynomials
