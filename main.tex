...
We note that an irreducible polynomial over a ring always exists~\cite[Remark 2.1.25]{Mullen:2013}. Evidently, an irreducible polynomial cannot be reduced, or divided into simpler polynomials. In the context of multivariate polynomial rings, division is ambiguous if further structure is not attached to the ring. This is discussed in detail in Section~\ref{sec:mult}.

However, in the context of multivariate polynomial rings, quotients and remainders of a division between two multivariate polynomials depend on which one is considered the divisor, that is, one that represents the ``largest quantity''. This concept is defined in the context of multivariate cryptography in Section~\ref{sec:mult}. We are now ready to define non-prime fields.
...
With the pure mathematical machinery defined, we now introduce the building blocks to create a generic digital signature scheme based on multivariate polynomial equations.

As mentioned in Section~\ref{sec:algebra}, division of multivariate polynomials is ambiguous if a structure is not given to order monomials.

not used in the definition of a non-prime field, but it is a pervasive concept in other aspects of multivariate cryptography. Thus, we take a short detour to make this clear by generalizing the aforementioned ordering for multivariate polynomials. 
...
The definition above amounts to ordering monomials according to their associated $n$-uples of exponents, which is more intuitive. 
