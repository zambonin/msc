...
We recall that $\widetilde{V}_{1}$ is the sequence of vinegar variables used on the signature generation algorithm of Rainbow. In the original scheme, it is chosen randomly for every signing process, yielding roughly $q^{v_{1}}$ possible preimages for a message digest. Without loss of generality, for Rainbow-$\eta$, there is exactly one preimage for every message digest if the partial evaluation of the central map gives an exactly determined polynomial system. We propose to interpret this quantity as a private parameter of the signature scheme. We describe the resulting variant scheme below.

Let $\mathbf{v} \in \mathbb{F}_{2}^{v_{1}}$ be the \emph{vinegar threshold}, a sequence of bits that encodes which elements from $\widetilde{V}_{1}$ are to be fixed. More precisely, for $1 \leq i \leq v_{1}$, if $\mathbf{v}_{i} = 0$ then the i-th element of $\widetilde{V}_{1}$ is chosen randomly from $\mathbb{F}_{q}$, and if $\mathbf{v}_{i} = 1$, a pre-computed value is picked. Evidently, if $\mathbf{v} = (0, \dots, 0)$, we have the original Rainbow scheme, and for $\mathbf{v} = (1, \dots, 1)$, we get Rainbow-$\eta$. The fact that every vinegar variable is fixed leads to the attack of Subsection~\ref{subsec:etaks}. However, if there is no information on which, and how many variables are fixed, the attacker is forced to guess such combinations, hampering its progress.
