...
\usepackage{amsfonts, amsthm, bm, mathtools, multirow}
...
\begin{lemma}[~\cite{}]
  Given a group $\mathsf{G}$, the set of all automorphisms carrying the usual function composition binary operation forms a group, denoted $\mathsf{Aut}(\mathsf{G})$.
\end{lemma}
...
We note that an irreducible polynomial over a ring always exists~\cite{}. Finite fields of the form $\mathbb{F}_{2^{l}}$ are especially useful in computer science, given that coefficients are restricted to $\{0, 1\}$ and may be represented by bit arrays, with arithmetic done through logic operations.
...
Such functions between coordinate spaces are usually represented by polynomial maps. A trivial example of a linear transformation is the identity function $\textsf{id} : \mathbf{V} \to \mathbf{V}$ such that $\textsf{id}(\mathbf{v_{1}}) = \mathbf{v_{1}}$. Moreover, if $\dim \mathbf{V} = \dim \mathbf{V'}$, there may exist an inverse function $\varphi^{-1} : \mathbf{V'} \to \mathbf{V}$ such that $\varphi \circ \varphi^{-1} = \textsf{id}$. We remark that Definitions~\ref{} and~\ref{} are analogous in the setting of vector spaces. Indeed, there exists an automorphism group $\textsf{Aut}(\mathbf{V})$, which is the set of all invertible linear transformations carrying usual function composition. In this case, this group is more commonly known as the \emph{general linear group} $\mathsf{GL}(\mathbf{V})$.
...
We observe that the $\mathsf{GL}_{n}(\mathbb{F})$ notation does not refer to a vector space. This is due to the remarkable fact that linear transformations may be interpreted as matrices over the base field of the vector space~\cite[Section 11.2]{}. Thus, map compositions and applications can be interpreted as matrix-matrix and matrix-vector multiplications throughout our work.

\section{Multivariate signature schemes}

\section{The Oil--Vinegar trapdoor}
...
\begin{align}
  \sharp\mathsf{sk} = \underbrace{m^{2} + m}_{\mathcal{S}} 
...
\begin{align}
  \sharp\mathsf{pk} = m \cdot \frac{(n + 1) \cdot (n + 2)}{2}.
\end{align}
...
\begin{multline}
  \sharp\mathsf{pk}^{\eta} = \underbrace{v_{1}}_{\widetilde{V}_{1}}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{*{2}{l}*{5}{r}}
    \toprule
    Instance & Parameters & $n$ & $m$ & $\sharp\mathsf{sk}$ & $\sharp\mathsf{sk}$ & Difference \\ \midrule
...
\begin{table}[htbp]
  ...
  \begin{tabular}{*{2}{l}*{5}{r}}
    \toprule
    Instance & Parameters & Variant & $\sharp\mathsf{sk}$ & $\sharp\mathsf{pk}^{\eta}$ & $\sharp\mathsf{pk}$ & Difference \\ \midrule
...
We compare the total key pair sizes $\sharp\mathsf{sk} + \sharp\mathsf{pk}$
...
$\sharp\mathsf{pk}$ for the variants according to Equations 9.2 and 9.4 of the
