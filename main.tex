...
\begin{definition}
  Given a finite field $\mathbb{F}$ with binary operations $+$ and $\times$, a set $V$, any elements $f_{1}, f_{2} \in \mathbb{F}$ and $\mathbf{u} \in V$, a binary operation $\mathbf{+}$ on $V$ and a binary operation $\cdot : \mathbb{F} \to V$, a \emph{vector space over $\mathbb{F}$} is an ordered $3$-uple $(V, +, \cdot)$ satisfying the following axioms:
  
  \begin{enumerate}
    \item The ordered pair $(V, \mathbf{+})$ forms an \emph{abelian group};
