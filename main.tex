...
Let this set be denoted as $\rho = (\sigma_{1}, \dots, \sigma_{m + 1})$. Any preimage of $\mathcal{C}$ always starts with $\widetilde{V}_{1} = (x_{1}, \dots, x_{v})$. Thus, by inverting the last linear transformation used to scramble the preimage, we obtain $\mathcal{L}_{2}(\sigma_{i}) = (x_{1}, \dots, x_{v}, \ast, \dots, \ast)$. Then, for any $1 \leq i, j \leq m + 1$ with $i \neq j$, we have
\begin{align}
