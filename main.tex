...
\begin{align}
  ...
  \end{dcases}
\end{align}
Rewriting the public polynomial of Equation~\ref{}, we have
\begin{align}
  \widetilde{h}^{(k)} = \sum_{i = 1}^{n} \sum_{j = i}^{n} \left( \alpha_{ij}^{(k)} \sum_{r = 1}^{n} \sum_{s = 1}^{n} b_{rs}^{ij} x_{r} x_{s} \right).
\end{align}
A single coefficient from that polynomial is thus exactly
\begin{align}
  \nu_{ij}^{(k)} = \sum_{i = 1}^{n} \sum_{j = i}^{n} \alpha_{ij}^{(k)} b_{rs}^{ij}.
\end{align}

%  Since $\mathcal{L}_{2}$ simply permutes coefficients maintaining the structure of $\mathcal{C}$, we let $\mathcal{Q} = \mathcal{C} \circ \mathcal{L}_{2}$, and consequently $\mathcal{P} = \mathcal{L}_{1} \circ \mathcal{Q}$.

By fixing a specific $\mathcal{L}_{2}$, Equation~\ref{} becomes a linear relation between the coefficients of the private and public keys. 
