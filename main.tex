...
\begin{definition}
  Given a finite field $\mathbb{F}$ equipped with binary operations $+$ and $\times$, a set $V$, any elements $f_{1}, f_{2} \in \mathbb{F}, \mathbf{v_{1}}, \mathbf{v_{2}} \in V$, a binary operation $\oplus$ on $V$ and a binary operation $\cdot : \mathbb{F} \to V$, a \emph{vector space over $\mathbb{F}$} is an ordered $3$-uple $(V, \oplus, \cdot)$ satisfying the following axioms:
  
  \begin{enumerate}
    \item The ordered pair $(V, \oplus)$ forms an \emph{abelian group};
    \item The binary operation $\cdot$ is \emph{associative}, that is, $f_{1} \cdot (f_{2} \cdot \mathbf{v_{1}}) = (f_{1} \times f_{2}) \cdot \mathbf{v_{1}}$;
    \item The binary operation \emph{$\cdot$ left distributes over $\oplus$}, that is, we must have that $f_{1} \cdot (\mathbf{v_{1}} \oplus \mathbf{v_{2}}) = (f_{1} \cdot \mathbf{v_{1}}) \oplus (f_{1} \cdot \mathbf{v_{2}})$;
    \item The binary operation \emph{$\oplus$ left distributes over $\cdot$}, that is, we must have that $(f_{1} + f_{2}) \cdot \mathbf{v_{1}} = (f_{1} \cdot \mathbf{v_{1}}) \oplus (f_{2} \cdot \mathbf{v_{1}})$.
  \end{enumerate}
\end{definition}

Vector spaces are the object of study of linear algebra. The members of $V$ are usually called \emph{vectors}, and the members of $\mathbb{F}$ are \emph{scalars}. It follows that the binary operation $\cdot$ is called \emph{scalar multiplication}. Given $c \in \mathbb{N}$, if members of $V$ are $c$-uples of elements of $\mathbb{F}$, the resulting vector space is called a \emph{coordinate space} and usually represented by $\mathbb{F}^{c}$.

\begin{definition}
  Given a vector space $(V, \oplus, \cdot)$ over $\mathbb{F}$, $\mathbf{v_{1}}, \dots, \mathbf{v_{n}} \in V$ and $f_{1}, \dots, f_{n} \in \mathbb{F}$, if the unique solution for the linear combination $f_{1} \cdot \mathbf{v_{1}} \oplus \dots \oplus f_{n} \cdot \mathbf{v_{n}} = 0$ is $f_{1} = \cdots = f_{n} = 0$, then the vectors $\mathbf{v_{1}}, \dots, \mathbf{v_{n}}$ are said to be \emph{linearly independent}.
\end{definition}

\begin{definition}
  Given a vector space $(V, \oplus, \cdot)$ over $\mathbb{F}$ and $\mathbf{v_{1}}, \dots, \mathbf{v_{n}} \in V$, the set of all linear combinations $f_{1} \cdot \mathbf{v_{1}} \oplus \dots \oplus f_{n} \cdot \mathbf{v_{n}}$, where each $f_{i}, \dots, f_{n}$ is any element in $\mathbb{F}$, is called the \emph{linear span}.
\end{definition}

\begin{definition}
  Given a vector space $(V, \oplus, \cdot)$ over $\mathbb{F}$ and $B = \{\mathbf{v_{1}}, \dots, \mathbf{v_{n}}\}$ with $B \subset V$, if the elements of $B$ are linearly independent and the linear span of $B$ is $V$, then $B$ is called a \emph{basis of the vector space}.
\end{definition}

% topics for tomorrow: polynomial maps, vector spaces
% topics for later: matrices, general linear group, agl, matrix representations of certain polynomials
