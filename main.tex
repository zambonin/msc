...
\begin{definition}
  Given $n \in \mathbb{N}$, a set of expressions $A$ with $a_{0}, \dots, a_{n} \in A$ called \emph{coefficients} and a \emph{variable} $x$, the expression $$\sum_{i = 0}^{n} a_{i} x^{i} = a_{n} \cdot x^{n} + \dots + a_{2} \cdot x^{2} + a_{1} \cdot x + a_{0}$$ is called an \emph{univariate polynomial}.
\end{definition}
