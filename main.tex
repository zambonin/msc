...
Oftentimes it is convenient to interpret elements as those of a different group. However, the binary operation has to be ``preserved'' throughout this mapping. We precisely define this below.
...
Such functions between coordinate spaces are usually represented by polynomial maps. A trivial example of a linear transformation is the identity function $\textsf{id} : \mathbf{V} \to \mathbf{V}$ such that $\textsf{id}(\mathbf{v_{1}}) = \mathbf{v_{1}}$. Moreover, if $\dim \mathbf{V} = \dim \mathbf{V'}$, there may exist an inverse function $\varphi^{-1} : \mathbf{V'} \to \mathbf{V}$ such that $\varphi \circ \varphi^{-1} = \textsf{id}$. We remark that Definitions~\ref{} and~\ref{} are analogous in the setting of vector spaces. Indeed, there exists an automorphism group $\textsf{Aut}(\mathbf{V})$, which is the set of all invertible linear transformations carrying usual function composition. This group is more commonly known as the \emph{general linear group} $\mathsf{GL}(\mathbf{V})$. 

\begin{lemma}
  Consider a vector space $\mathbf{V}$ over $\mathbb{F}$ with $\dim(\mathbf{V}) = n$. Let the general linear group of degree $n$ $\mathsf{GL}_{n}(\mathbb{F})$ be a group composed of  the set of invertible matrices of order $n \times n$ with elements in $\mathbb{F}$ carrying the usual matrix multiplication binary operation
\end{lemma}
For finite vector spaces of dimension $n$, this group is isomorphic to the  ~\cite{}, . We observe that this notation does not include any reference to a vector space. This is due to the remarkable fact that linear transformations may be interpreted as matrices over the base field of the vector space~\cite[Section 11.2]{}.
