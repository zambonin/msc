...
\begin{definition}
  Given a finite field $\mathbb{F}$ of \emph{scalars}, a set $V$, elements $f_{1} \in \mathbb{F}$ and $\mathbf{u} \in V$, a binary operation $+$ on $V$ and a binary operation $\cdot : \mathbb{F} \to V$, a \emph{vector space over $\mathbb{F}$} is an ordered $3$-uple $(V, +, \cdot)$ satisfying the following axioms:
  
  \begin{enumerate}
    \item The ordered pair $(V, +)$ forms an \emph{abelian group};
    \item 
  \end{enumerate}
\end{definition}
