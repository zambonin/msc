...
We will depart from using $e$ as the identity element, for it is not clear which operation it refers to. We hereafter refer to $0$ as the \emph{additive identity} and to $1$ as the \emph{multiplicative identity}. Analogously, given a ring $(R, +, \times)$ and for any $r_{1} \in R$, we denote $-r_{1}$ as the \emph{additive inverse of $r_{1}$}, and $r_{1}^{-1}$ as the \emph{multiplicative inverse of $r_{1}$}.
