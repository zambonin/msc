...
Further, it is argued in~\cite[Lemmata 3.2, 3.10, 3.19]{Thomae:201306} that some IP problems modelled to work with Oil--Vinegar signature schemes are no harder to solve if $\mathcal{L}_{1}$ and $\mathcal{L}_{2}$ are linear instead of affine transformations. As a consequence, such results need the polynomials in the central map to be homogeneous quadratic. In the case of Oil--Vinegar schemes, there is evidence that this approach is safe~\cite{}, but that may not be the case with other schemes built upon the bipolar construction.

We make use of this technical result and tailor our description of the bipolar construction to be coherent with the schemes presented in Section~\ref{sec:ov}.
