...
\begin{definition}
  Given a set $G$ and any $g_{1}, g_{2} \in G$, a binary operation $\star$ on a set $G$ is a function $\star : G \times G \to G$, where we denote $\star(g_{1}, g_{2})$ as $g_{1} \star g_{2}$. A binary operation $\star$ on a set $G$ may also satisfy the following conditions:
  
  \begin{itemize}
    \item It is said to be associative if, for any $g_{1}, g_{2}, g_{3} \in G$, then $g_{1} \star (g_{2} \star g_{3}) = (g_{1} \star g_{2}) \star g_{3}$.
    \item It is said to be commutative if all elements of $G$ commute, that is, for any $g_{1}, g_{2} \in G$, we have that $g_{1} \star g_{2} = g_{2} \star g_{1}$.
  \end{itemize}
\end{definition}
