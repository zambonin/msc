...
\begin{align}
  \begin{split}
    \mathbf{sk} &= (\mathcal{L}_{1}, \mathcal{C}, \mathcal{L}_{2})
      \:\quad \in \mathsf{GL}_{m}(\mathbb{F}) \times \mathsf{MHQ}(n, m) \times \mathsf{GL}_{n}(\mathbb{F}), \\
...
\begin{align}
  \begin{split}
    \mathbf{sk} &= (\mathcal{C}, \mathcal{L}_{2})
      \:\quad \in \mathsf{OVH}(n, m) \times \mathsf{GL}_{m}(\mathbb{F}), \\
    \mathbf{pk} &= \mathcal{L}_{1} \circ \mathcal{C} \circ \mathcal{L}_{2} 
      \quad \in \mathsf{MHQ}(n, m).
  \end{split}
\end{align}
...
Choose a central map $\mathcal{C} : \mathbb{F}^{n} \to \mathbb{F}^{m} \random \mathsf{RB}(v_{1}, o_{1}, \dots, o_{u})$. This random choice is equivalent to randomly choosing polynomial systems from each $\mathsf{OVH}(v_{\ell}, o_{\ell})$ with $1 \leq l \leq u$, analogous to multiple UOV systems. Since each sequence of vinegar variables in a layer contains all oil and vinegar variables from the previous layer, this allows for the inversion of this map, as presented further along this description. Let $\mathcal{L}_{1} \random \mathsf{GL}_{m}(\mathbb{F}), \mathcal{L}_{2} \random \mathsf{GL}_{n}(\mathbb{F})$ and $\mathcal{P} = \mathcal{L}_{1} \circ \mathcal{C} \circ \mathcal{L}_{2}$. Then, $\mathsf{sk} = (\mathcal{L}_{1}, \mathcal{C}, \mathcal{L}_{2}) \in \mathsf{GL}_{m}(\mathbb{F}) \times \mathsf{RB}(v_{1}, o_{1}, \dots, o_{u}) \times \mathsf{GL}_{n}(\mathbb{F})$ and $\mathsf{pk} = \mathcal{P} \in \mathsf{MHQ}(n, m)$.
