...
\begin{definition}
  Given a vector space $(V, \oplus, \cdot)$ over $\mathbb{F}$, vectors $\mathbf{v_{1}}, \dots, \mathbf{v_{n}} \in V$ and scalars $f_{1}, \dots, f_{n}$, if the only solution for the equation $f_{1} \cdot \mathbf{v_{1}} + \dots + f_{n} \cdot \mathbf{v_{n}} = 0$ is $f_{1} = \cdots = f_{n} = 0$, then the vectors $\mathbf{v_{1}}, \dots, \mathbf{v_{n}}$ are said to be \emph{linearly independent}.
\end{definition}
