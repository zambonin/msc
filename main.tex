...
A polynomial system is \emph{consistent} if there exists at least one solution $x_{1}, \dots, x_{n}$, which may not even live in the base coefficient ring~\cite{}, and \emph{inconsistent} otherwise. Foot-finding techniques have been studied for centuries and there exist several different methods to calculate one or all roots. We will be mostly focused on systems of linear equations, in which all polynomials have degree at most one.
