...
We remark that the symmetric matrix of Definition~\ref{def:mathom} has a diagonal of zeroes if the underlying field characteristic is even. Now, we recall the definition of two essential building blocks for a multivariate signature scheme.

\begin{definition}
  Given $k \in \mathbb{N}$, a function $\mathcal{H} : X \to Y$, $\mathcal{H}$ is said to be a \emph{cryptographic hash function} if it satisfies the following conditions:
  
  \begin{enumerate}
    \item It is \emph{deterministic};
    \item It is \emph{preimage resistant}
  \end{enumerate}
  
  The set $X$ is usually the set of all binary words $\{0, 1\}^{*}$, while $Y$ is usually $\mathbb{F}_{2^{k}}$ for some small $k$.
\end{definition}

a cryptographic hash function, which is the usual method of transforming a variable-length message into a fixed-length digest, 
