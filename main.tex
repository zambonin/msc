...
\begin{definition}
  Let $\mathbb{Z}^{n}$ be the set of $n$-uples of integers and any $\alpha, \beta, \gamma in \mathbb{Z}^{n}$. Given a polynomial ring $\mathsf{R}[\mathbf{x}]$ and the set of monomials $\mathbf{x}^{\alpha} = x_{1}^{\alpha_{1}} x_{2}^{\alpha_{2}} \cdots x_{n}^{\alpha_{n}}$, a \emph{monomial ordering} is a well-order $\lesssim$ in $\mathbb{Z}^{n}$ such that $\alpha + \gamma \lesssim \beta + \gamma$ if $\alpha \lesssim \beta$.
\end{definition}
