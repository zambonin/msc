...
Our hypothesis is that $Z_{t}$ and $\widetilde{Z}_{t}$ behave similarly to observations sampled from a discrete uniform distribution. This is due to the random process used in choosing the parts of a private key. Indeed, every element of $\mathbb{F}$ is equally likely to be picked to compose a linear transformation or central map.
