...
\begin{definition}
  A polynomial map $\mathcal{C} : \mathbb{F}^{v + o} \to \mathbb{F}^{o}$ consisting of $p^{(1)}, \dots, p^{(o)} \in \mathsf{OVP}(v, o)$ is an \emph{Oil--Vinegar polynomial map}. The set of all such maps is defined as $\mathsf{OV}(n, m)$. If all polynomials are homogeneous quadratic, then the respective set is $\mathsf{OVH}(n, m)$.
\end{definition}
