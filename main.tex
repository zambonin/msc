...
It is argued in~\cite[Lemmata 3.2, 3.10, 3.19]{Thomae:201306} that some IP problems modelled to represent Oil--Vinegar schemes are no harder to solve if $\mathcal{L}_{1}$ and $\mathcal{L}_{2}$ are linear instead of affine transformations. As a consequence, the polynomials in the central map must be homogeneous quadratic. There is also independent evidence that this approach is safe~\cite{}, but that may not be the case with other schemes built upon the bipolar construction.
