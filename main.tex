...
Consequently, a scheme using the bipolar construction is also based on some variant of the \emph{isomorphism of polynomials problem}. There are several versions of this problem, and we give a short definition for the variant that Oil--Vinegar schemes are based on.
...
Finally, we remark that the lateral maps are invertible square matrices, in contrast to other descriptions of the bipolar construction, which have the restriction $\mathcal{L}_{1}, \mathcal{L}_{2} \in \mathsf{AGL}_{n}(\mathbb{F})$. This group is called the \emph{affine group}, which is composed of matrix-vector pairs $(M, \mathbf{y})$ that encode the affine transformation $M\mathbf{x} \bm{+} \mathbf{y}$. By setting the translation vector $\mathbf{y} = (0, \dots, 0)$, we get a linear transformation. 

It is argued in~\cite[Lemmata 3.2, 3.10, 3.19]{Thomae:201306} that the underlying isomorphism of polynomials problems are no harder to solve if $\mathcal{L}_{1}$ and $\mathcal{L}_{2}$ are linear instead of affine transformations. We make use of this technical result and simplify the description of the bipolar construction as well as of schemes in Section~\ref{sec:ov}. Moreover, such results imply that the central map must be homogeneous quadratic.  
