...
We hereafter refer to $0$ as the \emph{additive identity} and to $1$ as the \emph{multiplicative identity} of a ring, if it exists. Analogously, for any element $r$ of a ring, we denote $-r$ as the \emph{additive inverse of $r$}, and $r^{-1}$ as the \emph{multiplicative inverse of $r$}, if it exists.
...
\begin{definition}
  Given a unit ring $(R, +, \times)$ and any $r_{1} \in R, r_{1} \neq 0$, if there exists a uniquely determined element $r_{1}^{-1} \in R$ such that $r_{1} \times r_{1}^{-1} = r_{1}^{-1} \times r_{1} = 1$, then the ring is called a \emph{division ring}.
\end{definition}

For instance, the ring of integers equipped with usual addition and multiplication $(\mathbb{Z}, +, \times)$ is a commutative unit ring,

\begin{definition}
  A commutative division ring is called a \emph{field}.
\end{definition}

% If it is clear from the context, we hereafter refer to group-like structures simply by the name of its base set and omit the associated binary operations. 
