...
An extension of the UOV-Reconciliation attack in the context of Rainbow~\cite{Ding:200806}, with the intent of producing an equivalent private key. It explores the fact that the central map matrix representation is composed of zeroes on its lower right corner. These yield quadratic equations which, if solved, lead to an equivalent private key. The complexity of this attack is given by the hardness of solving a polynomial system with $n$ variables and $m + n - 1$ equations~\cite[Theorem 3.30]{Thomae:201306}, which as seen above, is hard to estimate. It has been proposed by~\cite{Petzoldt:201005} that, in order to resist this attack, $n \geq \ceil{\frac{5 (m - 1)}{3}}$. Furthermore, a recent work~\cite{Perlner:202006} has shown improvements with regards to the complexity of this attack, inducing a slight increase in parameter values submitted to the NIST standardization process. This is also discussed in the following.
...
Recent developments of Rainbow cryptanalysis~\cite{Ding:202006} have improved the complexity of Rainbow-Band-Separation and MinRank attacks. Due to the lack of precise formulas for the case of Rainbow, we present the results in Table~\ref{tab:newsec} as given by the authors of the aforementioned works. We refer to the revised parameter sets simply by their targeted NIST security levels and convert the security level from logic gate counts to field multiplications, as per~\cite[p.~35]{Ding:201901}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{*{2}{l}*{5}{r}}
    ...
    I      & $(\mathbb{F}_{ 16}, 36, 32, 32)$  &  100 &   64 &   103616 &   27026 &  $-73.92\%$ \\
    III    & $(\mathbb{F}_{256}, 68, 32, 48)$  &  148 &   80 &   626016 &  107652 &  $-82.80\%$ \\
    V      & $(\mathbb{F}_{256}, 96, 36, 64)$  &  196 &  100 &  1408704 &  204384 &  $-85.49\%$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{*{2}{l}*{4}{r}}
    ...
    \multirow{3}{*}{P-128}  & Classic  &  \multirow{3}{*}{  96288} &  \multirow{3}{*}{  17754} &   135880 &  $-33.83\%$ \\
                            & Cyclic   &                           &                           &    44971 &  $-72.98\%$ \\
                            & LRS2     &                           &                           &    42107 &  $-74.22\%$ \\
    \multirow{2}{*}{I    }  & Classic  &  \multirow{2}{*}{ 103616} &  \multirow{2}{*}{  27026} &   161600 &  $-28.88\%$ \\
                            & nCyclic  &                           &                           &    60160 &  $-67.13\%$ \\
    \multirow{2}{*}{III  }  & Classic  &  \multirow{2}{*}{ 626016} &  \multirow{2}{*}{ 107652} &   882080 &  $-34.37\%$ \\
                            & nCyclic  &                           &                           &   264576 &  $-75.32\%$ \\
    \multirow{2}{*}{V    }  & Classic  &  \multirow{2}{*}{1408704} &  \multirow{2}{*}{ 204384} &  1930600 &  $-36.07\%$ \\
...
As discussed in Section~\ref{subsec:etaks}, the collection of signatures produced with Rainbow-$\eta$ is pivotal to carry on with the attack. Thus, we show the probabilities associated with amassing a large number of signatures and picking a valid subset of items to, in turn, obtain an equivalent key. We remark that there is no way to identify if a signature is produced with Rainbow-$\eta$ except by observation of the public implementations. However, it is simple enough to modify the key and signature generation algorithms to generate either conventional or reduced private keys at the will of the signer. 
...
By precisely choosing a target, an attacker may increase the odds of obtaining useful signatures for the attack. However, for all intents and purposes, we will observe the probability of carrying on with the attack for various values of $\alpha$. This behaviour is modelled as follows. Given the aforementioned $N, K$, we wish to obtain $k \leq K$ signatures that were produced with Rainbow-$\eta$. By the description of our attack, the attacker needs $k$ distinct signatures, or in other words, sampling is \emph{without replacement}. There exists a discrete probability distribution that precisely describes this situation.
...
The identification problem remains difficult if $n$ is much larger than $k$, since the attacker still does not know which signatures are useful to perform the attack. Ideally, a sample should return exactly the desired number of ``weak'' signatures, that is, $n = k$. The PMF would then be equal to
\begin{align}
  \frac{\binom{K}{k}}{\binom{N}{k}} = \frac{K! (N - k)!}{N! (K - k)!},
\end{align}
which gives negligible probabilities even for relatively small parameters. For example, given $N = 2^{8}$ and $K = 2^{7}$, or $\alpha = 0.5$, and $n = k = 2^{6}$, $\Pr[\mathbf{X} = k] \approx 1.2576 \times 10^{-24}$.

If the attacker collects signatures from multiple signers but cannot relate the signatures to each public key, the problem can still be modelled through a \emph{multivariate hypergeometric distribution}.
...
% It appears that partially fixing vinegar variables leads to a reasonable tradeoff, since for a large enough finite field and amount of random vinegar variables, the probability of obtaining vectors in the oil subspace is negligible. Furthermore, the effort of collecting signatures from multiple sources is non-trivial, and several combinations of signatures must be tried to obtain $\rho$ for a single private key. However, in the case of TLS, a number of signatures can be collected from a single source, that is, a server in the ephemeral Diffie--Hellman handshake step. 
