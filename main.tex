...
Finally, we obtain a \emph{vector subspace of $\mathbf{V}$} by applying the rationale of Definition~\ref{def:subgroup}. Observing how a linear transformation acts on a subspace leads to the following definition.

\begin{definition}
  Consider a vector space $\mathbf{V}$ over $\mathbb{F}$, a linear transformation $\phi : \mathbb{V} \to \mathbb{V}$, and a vector subspace $\mathbf{W}$ of $\mathbf{V}$. For every $\mathbf{w}$, if $\$
\end{definition}
