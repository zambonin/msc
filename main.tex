...
\usepackage{amsfonts, amssymb, amsthm, bm, mathtools, multirow}
...
We now turn to the main construction used in multivariate cryptography. It is
called bipolar, due to its elegant twofold nature. Consider
$n, m \in \mathbb{N}$ as in Definition~\ref{}. The polynomial map
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \draw[-latex] (z) -- (5, -1) -- (-2.5, -1) 
      node[midway,above] {$\mathcal{P}$} node[midway,yshift=-.5cm] {encryption / signature verification} -- (h);
  \end{tikzpicture}
...
The bipolar construction creates a specific situation in which the schemes
