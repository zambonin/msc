...
\begin{align}
  \begin{split}
    \mathbf{sk} &= (\mathcal{L}_{1}, \mathcal{C}, \mathcal{L}_{2}) 
      \in \mathsf{GL}_{m}(\mathbb{F}) \times \mathsf{MHQ}(n, m) \times \mathsf{GL}_{n}(\mathbb{F}), \\
...
As discussed in Section~\ref{sec:mult}, we choose the lateral maps to be linear transformations, since it is proven that this also does not affect the security of Oil--Vinegar schemes~\cite[Section 3.1]{Braeken:200502}. The same rationale occurs for homogeneous quadratic Oil--Vinegar polynomials. Thus, $\mathcal{L}_{2} \random \mathsf{GL}_{n}(\mathbb{F})$, or in other words, generate random matrices until one is invertible. Then, $\mathcal{P} = \mathcal{C} \circ \mathcal{L}_{2}$ and 
\begin{align}
  \begin{split}
    \mathbf{sk} &= (\mathcal{C}, \mathcal{L}_{2})
      \in \mathsf{OVH}(v, o) \times \mathsf{GL}_{n}(\mathbb{F}), \\
...
Consider a cryptographic hash function $\mathcal{H} : \{0, 1\}^{*} \to \mathbb{F}^{m}$, any message $M \in \{0, 1\}^{*}$, and obtain the message digest $d = \mathcal{H}(M)$. The signature is the set of variables
