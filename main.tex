...
The probability of sampling a valid subset of signatures is then equivalent to calculating $\Pr[\mathbf{X} > k]$, which is the description of the cumulative distribution function. We remark that for large populations, the naive computation $\sum_{i = k + 1}^{K} \Pr[\mathbf{X} = i]$ involves the calculation of large binomial coefficients, which may be computationally expensive and prone to overflow. We refer to 
