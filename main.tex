...
The linear transformations $\tau^{-1}$ and $\omega^{-1}$ maintain their forms by a similar block inverse calculation.
We use a compact notation to represent operations between inner block matrices. For example, $-(\tau^{(3)})^{-1} \cdot \tau^{(2)} \cdot (\tau^{(1)})^{-1}$ is denoted as $\tau^{-3^{-1} \cdot 2 \cdot 1^{-1}}$.
% \begin{align}
%   \begin{split}
%     \psi(\tau^{-1}) &=
%     \begin{bmatrix}
%       (\tau^{(1)})^{-1}                                           & \mathbf{0}_{o_{1} \times o_{2}} \\
%       -(\tau^{(3)})^{-1} \cdot \tau^{(2)} \cdot (\tau^{(1)})^{-1} & (\tau^{(3)})^{-1} \\
%     \end{bmatrix}, \\
%     \psi(\omega^{-1}) &=
%     \begin{bmatrix}
%       (\omega^{(1)})^{-1} & \mathbf{0}_{v_{1} \times o_{1}}   & \mathbf{0}_{v_{1} \times o_{2}} \\
%       -(\omega^{(3)})^{-1} \cdot \omega^{(2)} \cdot (\omega^{(1)})^{-1} & (\omega^{(3)})^{-1} & \mathbf{0}_{o_{1} \times o_{2}} \\
%       (\omega^{(6)})^{-1} \cdot -\omega^{(5)} \cdot -(\omega^{(3)})^{-1} \cdot \omega^{(4)} \cdot -(\omega^{(3)})^{-1} \cdot \omega^{(2)} \cdot (\omega^{(1)})^{-1} & -(\omega^{(6)})^{-1} \cdot \omega^{(5)} \cdot (\omega^{(3)})^{-1} & (\omega^{(6)})^{-1} \\
%     \end{bmatrix}.
%   \end{split}
% \end{align}
...
\begin{theorem}[{\cite[Theorem 3.6]{Petzoldt:201307}}]
  ...
  \begin{enumerate}
    ...
    \begin{align}
      \psi(\tau) =
      \begin{bmatrix}
        \mathcal{L}_{1}^{(1)} - \mathcal{L}_{1}^{(2)} \cdot (\mathcal{L}_{1}^{(4)})^{-1} \cdot \mathcal{L}_{1}^{(3)} & \mathbf{0}_{o_{1} \times o_{2}} \\
        \mathcal{L}_{1}^{(3)} & \mathcal{L}_{1}^{(4)} \\
      \end{bmatrix}
    \end{align}
    ...
    \begin{align}
      ...
      \begin{bmatrix}
        \mathbf{I}_{o_{1}} & \mathcal{L}_{1}^{(2)} \cdot (\mathcal{L}_{1}^{(4)})^{-1} \\
        \mathbf{0}_{o_{2} \times o_{1}} & \mathbf{I}_{o_{2}} \\
      \end{bmatrix}.
    \end{align}
    \item Let $\mathsf{L}_{2}^{(-1, 2)} = (\mathcal{L}_{2}^{(1)})^{-1} \cdot \mathcal{L}_{2}^{(2)}$ and $\mathsf{L}_{2}^{(-1, 3)} = (\mathcal{L}_{2}^{(1)})^{-1} \cdot \mathcal{L}_{2}^{(3)}$. Given $\omega \in \mathsf{GL}(\mathbb{F}^{n})$ such that
    \begin{align}
      \begin{split}
        \psi(\omega) =
        \begin{bmatrix}
          \mathcal{L}_{2}^{(1)} & \mathbf{0}_{v_{1} \times o_{1}} & \mathbf{0}_{v_{1} \times o_{2}} \\
          \mathcal{L}_{2}^{(4)} & \mathcal{L}_{2}^{(5)} - \mathcal{L}_{2}^{(4)} \cdot \mathsf{L}_{2}^{(-1, 2)} & \mathbf{0}_{o_{1} \times o_{2}} \\
          \mathcal{L}_{2}^{(7)} & \mathcal{L}_{2}^{(8)} - \mathcal{L}_{2}^{(7)} \cdot \mathsf{L}_{2}^{(-1, 2)} & \omega^{(6)} \\
        \end{bmatrix}, \\
        \begin{multlined}[t]
          \omega^{(6)} = \mathsf{L}_{2}^{((9 - 7 \cdot 1^{-1} \cdot 3) -(8 - 7 \cdot 1^{-1} \cdot 2) \cdot (5 - 4 \cdot 1^{-1} \cdot 2)^{-1} \cdot (6 - 4 \cdot 1^{-1} \cdot 3))}
        \end{multlined}
      \end{split}
    \end{align}
    ...
    \begin{align}
      ...
      \begin{bmatrix}
        \mathbf{I}_{v_{1}} & \mathsf{L}_{2}^{(-1, 2)} & \mathsf{L}_{2}^{(-1, 3)} \\
        \mathbf{0}_{o_{1} \times v_{1}} & \mathbf{I}_{o_{1}} & (\omega^{(3)})^{-1} \cdot \mathcal{L}_{2}^{(6)} - \mathcal{L}_{2}^{(4)} \cdot \mathsf{L}_{2}^{(-1, 3)} \\
