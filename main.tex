...
\begin{definition}
  Given a finite field $\mathbb{F}$ with binary operations $+$ and $\times$, a set $V$, any elements $f_{1}, f_{2} \in \mathbb{F}, \mathbf{v_{1}}, \mathbf{v_{2}} \in V$, a binary operation $\oplus$ on $V$ and a binary operation $\cdot : \mathbb{F} \to V$, a \emph{vector space over $\mathbb{F}$} is an ordered $3$-uple $(V, \oplus, \cdot)$ satisfying the following axioms:
  ...
  \begin{enumerate}
    ...
    \item The binary operation \emph{$\oplus$ left distributes over $\cdot$}, that is, we must have that $(f_{1} + f_{2}) \cdot \mathbf{v_{1}} = (f_{1} \cdot \mathbf{v_{1}}) \oplus (f_{2} \cdot \mathbf{v_{2}})$
  \end{enumerate}
\end{definition}
