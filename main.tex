...
\end{align}
which yields negligible probabilities even for relatively small parameters. For example, given $N = 2^{8}$ and $K = 2^{7}$, or $\alpha = 0.5$, and $n = k = 2^{6}$, $\Pr[\mathbf{X} = k] \approx 1.2576 \times 10^{-24}$. Hence, on average, it is not trivial to collect signatures and attack a Rainbow instance with sufficient security level. 
...
We infer that the success of an attacker is directly proportional to the maximization of the number of draws or the $\alpha$ factor. The chances of efficiently obtaining a collection of signatures useful to perform the attack is not negligible, but the effort associated with guessing such a subset is meaningful. Still, by bounding the security level of the resulting scheme to the chances associated with a best case scenario of the probabilities previously discussed, we clearly obtain an efficient, polynomial time procedure.
...
We analyzed the effort needed to collect signatures from an active target that randomly produces signatures with a conventional private key, or the same key minified with the aid of our proposal.
