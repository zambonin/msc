...
To verify a signature, compute $d' = \mathcal{P}(\mathbf{z})$. If $d = d'$, then the signature is valid, and otherwise invalid.
...
To verify a signature, compute $\mathbf{h'} = \mathcal{P}(\mathbf{z})$. If $\mathbf{h} = \mathbf{h'}$, then
