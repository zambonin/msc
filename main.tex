...
\begin{lemma}[{\cite[Lemma~1]{Kipnis:199904}}]
  Given any polynomials $f^{(i)}, f^{(j)} \in \textsf{OVH}(v, o)$, let $\phi^{(i)} : \mathbb{F}^{n} \to \mathbb{F}^{n}$ be the linear transformation associated to the matrix $\widetilde{M}(f^{(i)})$, and respectively for $\phi^{(j)}$. If $\widetilde{M}(f^{(j)})$ is invertible, then $\mathcal{O}$ is an invariant subspace of $\phi^{(i)} \circ (\phi^{(j)})^{-1}$.
\end{lemma}

An alternative description is that any polynomial of $\mathcal{C}$ maps $\mathcal{O}$ to $\mathcal{V}$.
We recall that $f^{(i)}$ and $f^{(j)}$ are part of an Oil--Vinegar central map. This fact does not change for any combination of polynomials of $\mathcal{P}$, as argued in~\cite[p.~261]{Kipnis:199808}.
