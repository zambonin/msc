...
Unfortunately, there is little information on the difficulty of EIP~\cite[p. 69]{Thomae:201306}, preventing complete security proofs for Oil--Vinegar schemes. This is also the case for other IP problems, such that most multivariate cryptosystems simply have \emph{ad hoc} security. Consequently, there exist many private keys that compose a single public key~\cite{Wolf:201104}, and the theory of \emph{equivalent keys} has been used to attack several schemes, as seen in Section~\ref{sec:related}. This concept is better explained alongside specific schemes, and thus it is done on Section~\ref{sec:ov}.
