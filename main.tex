...
\begin{definition}
  Given a finite field $\mathbb{F}$ with binary operations $+$ and $\times$, a set $V$, any elements $f_{1}, f_{2} \in \mathbb{F}$ and $\mathbf{u} \in V$, a binary operation $\oplus$ on $V$ and a binary operation $\cdot : \mathbb{F} \to V$, a \emph{vector space over $\mathbb{F}$} is an ordered $3$-uple $(V, \oplus, \cdot)$ satisfying the following axioms:
  
  \begin{enumerate}
    \item The ordered pair $(V, \oplus)$ forms an \emph{abelian group};
    \item The binary operation $\cdot$ is \emph{associative}, that is, $f_{1} \cdot (f_{2} \cdot \textbf{u}) = (f_{1} \times f_{2}) \cdot \textbf{u}$;
    \item The binary operation $\cdot$
  \end{enumerate}
\end{definition}
