...
This group is more commonly known as the \emph{general linear group} $\mathsf{GL}(\mathbf{V})$. For finite vector spaces of dimension $n$, this group is isomorphic to the general linear group of degree $n$ $\mathsf{GL}_{n}(\mathbb{F})$~\cite{}, the set of invertible matrices of order $n \times n$ with elements in $\mathbb{F}$ carrying the usual matrix multiplication binary operation. We observe that this notation does not include any reference to a vector space. This is due to the remarkable fact that linear transformations may be interpreted as matrices over the base field of the vector space~\cite[Section 11.2]{}.
