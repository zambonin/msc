...
\begin{definition}
  Given $n \in \mathbb{N}$, a set of expressions $A$ with $a_{0}, \dots, a_{n} \in A$ called \emph{coefficients} and a \emph{variable} $x$, the expression $$\sum_{i = 0}^{n} a_{i} x^{i} = a_{n} \cdot x^{n} + \dots + a_{2} \cdot x^{2} + a_{1} \cdot x + a_{0}$$ is called an \emph{univariate polynomial}. Each of the terms in the summation is called a \emph{monomial}. The largest $d$ such that $a_{d} \cdot x^{d}, a_{d} \neq 0$ is called the \emph{degree} of the pooyln
\end{definition}
