...
\begin{definition}
  Given a polynomial system $\mathcal{P} \in \textsf{MQ}(m, n)$ and a monomial ordering, the matrix representation of a polynomial $p^{(i)}, 1 \leq i \leq m$ is given by
  \begin{align}
    \overline{M}(p^{(i)}) = 
    \begin{pmatrix}
      \alpha^{(k)}_{11} & \alpha^{(k)}_{12} & \alpha^{(k)}_{13} & \cdots & \alpha^{(k)}_{1n} & \beta^{(k)}_1 \\
                      0 & \alpha^{(k)}_{22} & \alpha^{(k)}_{23} & \cdots & \alpha^{(k)}_{2n} & \beta^{(k)}_2 \\
