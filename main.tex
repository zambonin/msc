...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item It is said to be \emph{commutative} if we have that $g_{1} \star g_{2} = g_{2} \star g_{1}$;
...
\begin{definition}\label{def:morph}
  Given two groups $\mathsf{G}$ and $\mathsf{H}$, if an homomorphism $\varphi : \mathsf{G} \to \mathsf{H}$ is a bijection, it is called an \emph{isomorphism}. If $\mathsf{G} = \mathsf{H}$, $\varphi$ is an \emph{endomorphism}. If the function is simultaneously an isomorphism and an endomorphism, it is called an \emph{automorphism}.
\end{definition}
...
\begin{definition}
  Given $k \in \mathbb{N}$, a set of expressions $A$ with $a_{0}, \dots, a_{k} \in A$ called \emph{coefficients} and an \emph{indeterminate} $x$, an expression $p(x)$ of the form
  \[
    \sum_{i = 0}^{k} a_{i} x^{i} 
      = a_{k} \cdot x^{k} + \dots + a_{2} \cdot x^{2} + a_{1} \cdot x + a_{0}
  \]
  is called an \emph{univariate polynomial}. Each of the terms in the summation is called a \emph{monomial}.
\end{definition}
...
\begin{definition}
  Given a function $\mathcal{H} : X \to Y$ and any $m_{1}, m_{2} \in X$, $\mathcal{H}$ is said to be a \emph{cryptographic hash function} if it satisfies the following conditions:
...
\begin{definition}
  Given $n \in \mathbb{N}$, any message $w \in \{0, 1\}^{*}$ and a $3$-tuple of probabilistic polynomial-time algorithms $(\textsc{Gen}, \textsc{Sig}, \textsc{Ver})$, it is said to be a \emph{digital signature scheme} if it satisfies
  \begin{align}
     \forall (\mathbf{sk}, \mathbf{pk}) \leftarrow \textsc{Gen}(1^{n}),
       \quad \Pr[\textsc{Ver}(\mathbf{pk}, w, \textsc{Sig}(\mathbf{sk}, w)) = 1] = 1.
  \end{align}
  The integer $n$ is called the \emph{security parameter}.
\end{definition}
...
We now turn to the most commonly framework in multivariate cryptography employed to build cryptosystems. It is called bipolar construction, due to its elegant twofold nature, which yields the possibility of building public-key encryption or signature schemes with the same idea. 

Consider $n, m \in \mathbb{N}$ the number of variables and equations as in Definition~\ref{def:quadsys}. A polynomial map $\mathcal{C} : \mathbb{F}^{n} \to \mathbb{F}^{m} \in \mathsf{MHQ}(n, m)$ is called the \emph{central map} and regarded as the trapdoor, that is, a function that is easy to compute but difficult to invert without particular information. Evidently, this mechanism shall be hidden to hinder malicious parties. Two \emph{lateral maps} $\mathcal{L}_{1} : \mathbb{F}^{n} \to \mathbb{F}^{n} \in \mathsf{GL}_{n}(\mathbb{F})$ and $\mathcal{L}_{2} : \mathbb{F}^{m} \to \mathbb{F}^{m} \in \mathsf{GL}_{m}(\mathbb{F})$ are used to hide the structure of $\mathcal{C}$, by creating the composition $\mathcal{P} = \mathcal{L}_{1} \circ \mathcal{C} \circ \mathcal{L}_{2}$ that has no apparent structure, making recovery of the central map hard.
...
\begin{align}
  \begin{split}
    \mathbf{sk} &= (\mathcal{L}_{1}, \mathcal{C}, \mathcal{L}_{2})
      \:\quad \in \mathsf{GL}_{n}(\mathbb{F}) \times \mathsf{MHQ}(n, m) \times \mathsf{GL}_{m}(\mathbb{F}), \\
...
Finally, we remark that our description of the bipolar construction is slightly different from the literature, since our maps lie in more restricted sets. The lateral maps are invertible square matrices, in contrast to the usual restriction $\mathcal{L}_{1}, \mathcal{L}_{2} \in \mathsf{AGL}_{n}(\mathbb{F})$. This group is called the \emph{affine group}, which is composed of matrix-vector pairs $(M, \mathbf{y})$ that encode the affine transformation $M\mathbf{x} \bm{+} \mathbf{y}$. By setting the translation vector $\mathbf{y} = (0, \dots, 0)$, we get a linear transformation. It is argued in~\cite[Lemmata 3.2, 3.10, 3.19]{Thomae:201306} that the underlying IP problems are no harder to solve if $\mathcal{L}_{1}$ and $\mathcal{L}_{2}$ are linear instead of affine transformations. Moreover, such results imply that the polynomials in the central map must be homogeneous quadratic. We make use of this technical result and modify the description of the bipolar construction as well as of schemes in Section~\ref{sec:ov}.
