...
\end{align}
Further, let the elements of $\psi(\mathcal{L}_{2})$ be denoted as $d_{ij}, 1 \leq i, j \leq n$. We recall that the function composition $\mathcal{P} = \mathcal{C} \circ \mathcal{L}_{2}$ can be represented as a matrix multiplication operation. The quadratic part of $h^{(k)}$ may be rewritten as
\begin{align}
