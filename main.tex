...
It is thus expected that a large enough number of signatures is collected for a reasonable chance of obtaining a valid sample. For example, given $N = 2^{12}$ and $K = 2^{10}$, or $\alpha = 0.25$, and $n = 2^{8}$, then $\hat{\mathbf{X}} = 64$, which is equal to the number of equations of a Rainbow instance with minimal security level, according to Table~\ref{tab:newsec}. The associated probability of obtaining such a number $k$ of successes is $\Pr[\mathbf{X} = 64] \approx 5.938\%$.
