...
\begin{definition}
  Given two groups $(G, \star)$ and $(H, \diamond)$ and any $g_{1}, g_{2} \in G$, a function $\varphi : G \to H$ is an \emph{homomorphism} if we have that $\varphi(g_{1} \star g_{2}) = \varphi(g_{1}) \diamond \varphi(g_{2})$. If $\varphi$ is a bijection, then it is invertible, or an \emph{isomorphism}.
\end{definition}
