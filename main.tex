...
We note that there is no need to provide a reconstruction procedure of $\overline{\mathcal{C}}$ as in the case of Rainbow-$\eta_{1}$, which needs a seed, and Rainbow-$\eta_{2}$, that features non-trivial mathematical relations. The private key size of Rainbow-$\eta_{4}$ is similar to Proposition~\ref{prop:eta-key-size}, increasing due to storage of $\mathbf{v}$, but reducing due to the size of $\widetilde{V}_{1}$, which is now $v_{1} - \text{wt}(\mathbf{v})$.
