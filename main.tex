...
\begin{definition}
  ...
  \begin{align}
    ...
    \end{split}
  \end{align}
  The set of all such polynomials is denoted by $\mathsf{MQ}(n, m)$.
\end{definition}
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item The ordering given by $\mathbf{x}^{\alpha} \lesssim \mathbf{x}^{\beta}$ if $\deg \mathbf{x}^{\alpha} \lesssim \deg \mathbf{x}^{\beta}$ or, as a tie-breaker, $\mathbf{x}^{\alpha} \lesssim_{\mathsf{lex}} \mathbf{x}^{\beta}$ is called \emph{graded lexicographic ordering}, and denoted as \textsc{grlex};
    \item The ordering given by $\mathbf{x}^{\alpha} \lesssim \mathbf{x}^{\beta}$ if $\deg \mathbf{x}^{\alpha} \lesssim \deg \mathbf{x}^{\beta}$ or, as a tie-breaker, 
  \end{itemize}
\end{definition}

There are several examples of ordering strategies~\cite{}. For instance, in lexicographic ordering (\textsf{lex}) the exponents of $x_{1}$ are compared and only if they are equal exponents of $x_{2}$ are compared, and so on. In graded lexicographic ordering (\textsf{grlex}), the monomial degree is used as first comparison and \textsc{lex} as tie-breaker. Finally, in graded reverse lexicographic ordering (\textsf{grevlex}) works similarly to \textsf{grlex} but the tie-breaker occurs by checking if the last element of 

For efficiency reasons~\cite{}, \textsf{grevlex} is more commonly employed. In the context of multivariate cryptography, monomial ordering is used for instance to define matrix representations of multivariate polynomials, and to solve polynomial systems using Gröbner bases~\cite{}. Additionally, as seen in Section~\ref{}, the visualization and manipulation of multivariate polynomials as matrices is pivotal in understanding and even attacking some cryptosystems. 
