...
\begin{definition}
  Given $f \in \mathbb{F}_{q}[x]$ irreducible with $\deg(f) = \ell$, then the finite field $\mathbb{F}_{q^{\ell}}$ is the set of polynomials with coefficients in $\mathbb{F}_{q}$ and $\deg(\cdot) < \ell$ carrying the usual polynomial arithmetic operations modulo $f$.
\end{definition}
