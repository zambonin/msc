...
\begin{definition}
  Given a finite field $\mathbb{F}$ equipped with binary operations $+$ and $\times$, a set $V$, any elements $f_{1}, f_{2} \in \mathbb{F}, \mathbf{v_{1}}, \mathbf{v_{2}} \in V$, a binary operation $\ding{58}$ on $V$ and a binary operation $\ast : \mathbb{F} \to V$, a \emph{vector space over $\mathbb{F}$} is an ordered $3$-uple $(V, \oplus, \ast)$ satisfying the following axioms:
  
  \begin{enumerate}
    \item The ordered pair $(V, \oplus)$ forms an \emph{abelian group};
    \item The binary operation $\ast$ is \emph{associative}, that is, $f_{1} \ast (f_{2} \ast \mathbf{v_{1}}) = (f_{1} \times f_{2}) \ast \mathbf{v_{1}}$;
    \item The binary operation \emph{$\ast$ left distributes over $\oplus$}, that is, we must have that $f_{1} \ast (\mathbf{v_{1}} \oplus \mathbf{v_{2}}) = (f_{1} \ast \mathbf{v_{1}}) \oplus (f_{1} \ast \mathbf{v_{2}})$;
    \item The binary operation \emph{$\oplus$ left distributes over $\ast$}, that is, we must have that $(f_{1} + f_{2}) \ast \mathbf{v_{1}} = (f_{1} \ast \mathbf{v_{1}}) \oplus (f_{2} \ast \mathbf{v_{1}})$.
  \end{enumerate}
\end{definition}

Vector spaces are the object of study of linear algebra. The members of $V$ are usually called \emph{vectors}, and the members of $\mathbb{F}$ are \emph{scalars}. It follows that the binary operation $\ast$ is called \emph{scalar multiplication}. Given $c \in \mathbb{N}$, if members of $V$ are $c$-uples of elements of $\mathbb{F}$, the resulting vector space is called a \emph{coordinate space} and usually represented by $\mathbb{F}^{c}$. We can thus recall Definition~\ref{} and create a function of several polynomials.

\begin{definition}
  Given $n, m \in \mathbb{N}$ and a finite field $\mathbb{F}$, a \emph{polynomial map} $p : \mathbb{F}^{n} \times \mathbb{F}^{m}$
\end{definition}

\begin{definition}
  Given a vector space $(V, \oplus, \ast)$ over $\mathbb{F}$, $\mathbf{v_{1}}, \dots, \mathbf{v_{n}} \in V$ and $f_{1}, \dots, f_{n} \in \mathbb{F}$, if the unique solution for the linear combination $f_{1} \ast \mathbf{v_{1}} \oplus \dots \oplus f_{n} \ast \mathbf{v_{n}} = 0$ is $f_{1} = \cdots = f_{n} = 0$, then the vectors $\mathbf{v_{1}}, \dots, \mathbf{v_{n}}$ are said to be \emph{linearly independent}.
\end{definition}

\begin{definition}
  Given a vector space and $B = \{\mathbf{v_{1}}, \dots, \mathbf{v_{n}}\} \subseteq V$, the set of all linear combinations $S = \{ f_{1} \ast \mathbf{v_{1}} \oplus \dots \oplus f_{n} \ast \mathbf{v_{n}} \mid \forall f_{1}, \dots, f_{n} \in \mathbb{F} \}$ is called the \emph{linear span}. If the elements of $B$ are linearly independent and $S = V$, then $B$ is called a \emph{basis of the vector space}.
\end{definition}
