...
\begin{proposition}
  The size of a Rainbow-$\eta_{4}$ private key, denoted $\textsf{sk}^{\eta_{4}}_{\text{\emph{wt}}(\mathbf{v})}$, is
  \begin{multline}
    \#\textsf{sk}^{\eta_{4}}_{\text{\emph{wt}}(\mathbf{v})} = \underbrace{\text{\emph{wt}}(\mathbf{v})}_{\widetilde{V}_{1}}
      + \underbrace{m^{2}}_{\mathcal{L}_{1}}
      + \underbrace{n^{2}}_{\mathcal{L}_{2}} \\
      + \underbrace{\sum_{k = 1}^{u} o_{k} \cdot \left( \frac{(v_{k} - \text{\emph{wt}})(v_{k} - \text{\emph{wt}} + 1)}{2}
        + (v_{k} - \text{\emph{wt}}) \cdot o_{k} + (v_{k + 1} - \text{\emph{wt}}) + 1 \right)}_{\overline{\mathcal{C}}}.
  \end{multline}
  field elements.
\end{proposition}
