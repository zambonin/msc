...
The identification problem remains if $n$ is much larger than $k$, since the attacker still does not know which signatures are useful to perform the attack. Ideally, a sample should return exactly the desired number of ``weak'' signatures, that is, $n = k$. The PMF would then be equal to the ratio between $\binom{K}{k}$ and $\binom{N}{n}$, which gives negligible probabilities 
