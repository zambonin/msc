...
To appropriately interpret sampling of $\mathbf{v}$, we set $n = v_{1}$ and $p = 0.5$.
For this case, the standard deviation is simply $0.5 \sqrt{n}$. Any sampling may yield an undesirable vinegar threshold, that is, one which Hamming weight is maximized, such that few elements of $\widetilde{V}_{1}$ are random and thus easily guess


It is desirable that $\text{wt}(\mathbf{v})$ is not maximized, as seen in Table~\ref{tab:bin-prob}, which shows the probability of obtaining a signature with the same vinegar variables given several Hamming weights.
