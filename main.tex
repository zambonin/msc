...
\begin{theorem}[{\cite[Theorem 4.15]{Wolf:201104}}]
  Given a private key $(\mathcal{C}, \mathcal{L}_{2}) \in \mathsf{OV}(v, o) \times \mathsf{GL}_{n}(\mathbb{F})$, if $\omega \in \mathsf{GL}_{n}(\mathbb{F})$ has zeroes in its upper-right $v \times o$ portion, then $(\mathcal{C} \circ \omega, \omega^{-1} \circ \mathcal{L}_{2})$ is an equivalent private key.
\end{theorem}
