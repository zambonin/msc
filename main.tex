...
The algorithm $\textsc{Gen}$ outputs the key pair, two bit strings known as the signing key and verification key, or private and public keys. The algorithm $\textsc{Sig}$ outputs a signature
$\textsc{Gen}(1^{n}) \mapsto (\mathbf{sk}, \mathbf{pk})$, $\textsc{Sig}(\mathbf{sk}, w) \mapsto \sigma$ and $\textsc{Ver}(\mathbf{pk}, w, \sigma) \mapsto \{0, 1\}$,
