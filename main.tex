...
Such functions between coordinate spaces are usually represented by polynomial maps. A trivial example of a linear transformation is the identity function $\textsf{id} : \mathbf{V} \to \mathbf{V}$ such that $\textsf{id}(\mathbf{v_{1}}) = \mathbf{v_{1}}$. Moreover, if $\dim \mathbf{V} = \dim \mathbf{V'}$, there may exist an inverse function $\varphi^{-1} : \mathbf{V'} \to \mathbf{V}$ such that $\varphi \circ \varphi^{-1} = \textsf{id}$. We remark that Definitions~\ref{} and~\ref{} are analogous in the setting of vector spaces. Indeed, there exists an automorphism group $\textsf{Aut}(\mathbf{V})$, which is the set of all invertible linear transformations carrying usual function composition.

This group is more commonly known as the \emph{general linear group} $\mathsf{GL}(\mathbf{V})$. For finite vector spaces of dimension $n$, this group is isomorphic to the general linear group of degree $n$, or $\mathsf{GL}_{n}(\mathbb{F})$, the set of invertible $n \times n$ matrices with elements in $\mathbb{F}$. We observe that this notation does not include any reference to a vector space. This is due to the remarkable fact that linear transformations may be interpreted as matrices over the base field of the vector space~\cite[Section 11.2]{}.

% The usual arrangement of the standard basis yields the \emph{identity matrix}, hereafter denoted as $\mathbf{I}_{n}$. In fact, several structures previously defined can also be represented as matrices, which is useful when performing calculations with relatively abstract elements such as polynomials.

% topics for tomorrow: invertible matrix <-> invertible map, general linear group, matrix representations of certain polynomials
