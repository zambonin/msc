...
In terms of keys, the keypair is composed of a private $3$-tuple and the public composition
\begin{align}
  \mathbf{sk} &= (\mathcal{L}_{1}, \mathcal{C}, \mathcal{L}_{2}) \quad \in \mathsf{GL}_{n}(\mathbb{F}) \times \mathsf{MQ}(n, m) \times \mathsf{GL}_{m}(\mathbb{F}) \\
  \mathbf{pk} &= \mathcal{L}_{1} \circ \mathcal{C} \circ \mathcal{L}_{2} \quad \in \mathsf{MQ}(n, m).
\end{align}
