...
This approach is based on the fact that a private key owner is able to recover the original central map through the possession of the lateral maps and the public key. We make use of the linear relations first introduced in~\cite{Petzoldt:201006} and applied in the definition of the well-known CyclicRainbow scheme. We denote this variant as Rainbow-$\eta_{2}$. First, a brief explanation of the relations is given below, with the full rationale available in~\cite[Chapter 7]{Petzoldt:201307}.
...
This step does not change significantly from the original scheme. Consider a message $M$ and obtain its digest $\mathbf{h} = \mathcal{H}(M)$. Compute $\mathbf{y} = \mathcal{L}_{1}^{-1}(\mathbf{h})$, and attempt to generate the preimage $\mathcal{C}(\mathbf{x}) = \mathbf{y}$ by inverting every layer recursively. If any of the transitory systems are not solvable, the original central map is computed from the aforementioned mathematical relations, a new $\widetilde{V}_{1}$ is chosen and a new $\overline{\mathcal{C}}$ created. Finally, compute $\mathbf{z} = \mathcal{L}_{2}^{-1}(\mathbf{x})$.
...
process~\cite{Ding:201901} presents a scheme description that diverges
