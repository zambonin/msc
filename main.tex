...
$0 = v_{0} < v_{1} < \cdots < v_{u} < v_{u + 1} = n$. For the range $1 \leq l \leq u$, take the usual set
$V = \{1, \dots, n\}$ and define vinegar variables as
$V_{l} = \{1, \dots, v_{l}\}$, and oil variables $O_{l} = \{v_{l} + 1, \dots, v_{l + 1}\}$. Oil variables
are given by. We note that $V_{1} \subset \dots \subset V_{u} = V$ and $O_{l} = V_{l + 1} - V_{l}$. Let $m = n - v_{1}$. Now,
