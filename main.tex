...
As seen in Chapter~\ref{ch:eta}, several arguments were made in order to present support to the security of our Rainbow-$\eta$ proposal. The main argument lies in the fact that the inner workings of the scheme are not modified and a sideline modification is done instead. Nonetheless, as summarized in Section~\ref{sec:related}, there is encouraging evidence that adding further structure to the central map is dangerous to the security of Oil--Vinegar schemes. While we intentionally avoid this strategy with our proposal, we still present evidence that our scheme is not secure due to leakage of information from the private key through signatures.
...
\section{Aftermath and countermeasures}
