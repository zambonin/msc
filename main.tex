...
We give a short introduction to cryptosystems which are based on systems of multivariate equations. Recall that a polynomial is a mathematical expression that consists of indeterminates, also called variables, and coefficients that multiply these. Only addition, subtraction, multiplication and integer exponentiation operations are permitted within polynomials. If we denote a $n$-uple of variables as $\mathbf{x} = (x_{1}, \dots, x_{n})$, a polynomial in this sequence is denoted $p(\mathbf{x})$. Further, a polynomial equation is constructed if we set $p(\mathbf{x}) = 0$, and a polynomial system
