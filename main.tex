...
\begin{definition}
  Given $n \in \mathbb{N}$, a polynomial over a sequence of variables $\mathbf{x} = (x_{1}, \dots, x_{n})$ is called a \emph{multivariate polynomial}, described by the expression
  \[
    \sum_{i_{1}}^{k_{1}} \cdots \sum_{i_{n}}^{k_{n}} a_{i_{1}, \dots, i_{n}} x^{i_{1}, \dots, i_{n}}
  \]
  
  The highest sum of the exponents of the variables in a monomial with a non-zero coefficient is called the \emph{degree} of the polynomial.
\end{definition}
