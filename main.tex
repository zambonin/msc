...
\end{align}
Focusing on the inner summation, we can represent the coefficients of the matrix multiplication by $\psi(\mathcal{L}_{2})$ by letting
\begin{align}
  d_{rs}^{ij} =
  \begin{dcases}
...
\begin{align}
  \nu_{ij}^{(k)} = \sum_{i = 1}^{n} \sum_{j = i}^{n} \alpha_{ij}^{(k)} d_{rs}^{ij}.
\end{align}
