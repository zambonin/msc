...
\begin{definition}
  Given a polynomial system $\mathcal{P} \in \textsf{MQ}(m, n)$ and a monomial ordering, the matrix representation of a polynomial $p^{(i)} \in \mathcal{P}, 1 \leq i \leq m$ is given by
  \begin{align}
    \overline{M}(p^{(i)}) = 
    \begin{pmatrix}
    ...
    \end{pmatrix}
  \end{align}
  such that $\mathbf{x}^{\textsf{T}} \cdot \overline{M}(p^{(i)}) \mathbf{x}$ for $\mathbf{x} = (x_{1}, \dots, x_{n})^{\textsf{T}}$.
\end{definition}
