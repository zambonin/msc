...
A variant called LWRS~\cite{} consists of modifications with the intent of creating a scheme suitable for performance-restricted devices. The authors propose to drops the left affine transformation of the usual bipolar construction and applies the ``minus'' method~\cite[Subsection 3.2.1]{} to Rainbow in order to create a scheme suitable for performance-restricted devices. However, an extra layer is added to the scheme instance, and it is unclear if suggested parameters are directly comparable to a known instance of the conventional Rainbow signature scheme. % this actually reduces both keys in the key pair
