...
Finally, further work is needed on the security of Rainbow signature schemes with partially fixed vinegar variables. It is unknown if fixing a single vinegar variable immediately leads to a polynomial time attack which can foil the target scheme. If that is not the case, we argue that there is a case to search for the maximal number of fixed variables that respects a security threshold, since private key size reductions are linear in the number of fixed vinegar variables.
