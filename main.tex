...
% TODO expand
% -----8<-----
Consider the key pair of a Rainbow signature scheme as described in Equation~\ref{}. Since $\mathcal{L}_{2}$ simply permutes coefficients maintaining the structure of $\mathcal{C}$, we let $\mathcal{Q} = \mathcal{C} \circ \mathcal{L}_{2}$, and consequently $\mathcal{P} = \mathcal{L}_{1} \circ \mathcal{Q}$. Moreover, it is known that quadratic terms do not mix with linear and constant terms in the case of Rainbow, and thus we refer only to the quadratic part of any polynomial systems in our description. Per Equation~\ref{eq:ov}, let the quadratic part of the $k$-th polynomial $q \in \mathcal{Q}$ be denoted as
\begin{align}
  \widetilde{q} = \sum_{i = 1}^{n} \sum_{j = i}^{n} \alpha_{ij} x_{i} x_{j}.
\end{align}
Further, 
