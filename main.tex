...
Consider $\chi_{i} = \sigma_{i} - \sigma_{j}$, and by Definition~\ref{def:oil-subspace}, $\chi_{i} \in \widetilde{\mathcal{O}}$. This is an effective characterization of the target subspace in the UOV attack. Thus, the only remaining task is to build a basis $B$ of $\widetilde{\mathcal{O}}$, for which $m$ linearly independent $\chi_{i}$ must be collected. Let us call this set $\widetilde{\rho}$. We note that at least $m + 1$ signatures need to be collected to perform $m$ subtractions, in the case that all $\chi_{i}$ are immediately linearly independent. As seen in Subsection~\ref{subsec:invert}, it is expected that with high probability 
