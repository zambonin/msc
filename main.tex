...
With most mathematical machinery defined, we now introduce the building blocks to create a generic public-key cryptosystem based on multivariate polynomial equations, with particular focus on digital signature schemes. We firstly recall two essential building blocks of cryptography, with definitions taken from~\cite{} and~\cite{}.
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item The ordering given by $\mathbf{x}^{\alpha} \lesssim \mathbf{x}^{\beta}$ if $\deg \mathbf{x}^{\alpha} \lesssim \deg \mathbf{x}^{\beta}$ or, as a tie-breaker, the last non-zero element of $\alpha - \beta$ is negative is called \emph{graded reverse lexicographic ordering}, and denoted as \textsf{grevlex}.
  \end{itemize}
\end{definition}
