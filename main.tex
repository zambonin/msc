...
Concrete implementations of Rainbow-$\eta_{3}$ are tested on
...
The private key size for Rainbow-$\eta_{1}$ is exactly $\#\mathsf{sk}^{\eta}$ plus the size of the seed $\mathbf{S}$, which is usually a standard integral data type, and thus composed of a small number of bytes, e.g. eight. In the case of Rainbow-$\eta_{2}$, there is no difference in the private key size. For Rainbow-$\eta_{3}$, the length of the salt is taken to be $w = 128$ bits~\cite[p.~11]{Ding:201901}, and thus the representation of this integer in the underlying field is added to $\#\mathsf{sk}^{\eta}$. We remark that the public key size of Rainbow-$\eta_{3}$ also increases by this amount.
