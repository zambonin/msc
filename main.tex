...
% TODO expand
% -----8<-----
Consider the key pair of a UOV signature scheme as described in Equation~\ref{}. It is known that quadratic terms do not mix with linear and constant terms in the case of Oil--Vinegar signature schemes, and thus we refer only to the quadratic part of any polynomial systems in our description. Per Equation~\ref{eq:ov}, let the quadratic part of the $k$-th polynomial $c^{(k)} \in \mathcal{C}$, and respectively $h^{(k)} \in \mathcal{P}$, be denoted as
\begin{align}
