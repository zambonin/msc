...
\begin{theorem}
  Given a private key $(\mathcal{C}, \mathcal{L}_{2}) \in \mathsf{OV}(v, o) \times \mathsf{GL}_{n}(\mathbb{F})$. Let
  \begin{align}
    \mathcal{L}_{2} = 
    \begin{bmatrix}
...
Then, by dividing $\mathcal{L}_{2}$ into analogous blocks, we have
\begin{align}
  \omega^{-1} \circ \mathcal{L}_{2} &= 
  \begin{bmatrix}
    \omega_{v \times v}^{(1)} & \mathbf{0}_{v \times o} \\
  ...
  \end{bmatrix}
\end{align}
for $\mathcal{L}_{2}^{(1)} \in \mathsf{GL}_{v}(\mathbb{F})$ and $\omega^{(1)} = (\mathcal{L}_{2}^{(1)})^{-1}$
