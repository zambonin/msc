...
\begin{table}[htbp]
  ...
  \setlength{\tabcolsep}{7pt}
  \centering
  \caption{Probability of guessing a signature with a previously used $\mathbf{v}$ depending on its Hamming weight.}\label{tab:bin-prob}
  \begin{tabular}{*{7}{r}}
    \toprule
    $q$ & $v_{1}$ & $\mu + 0\sigma$ & $\mu + \frac{1}{2}\sigma$ & $\mu + \sigma$ & $\mu + \frac{3}{2}\sigma$ & $\mu + 2\sigma$ \\
    \midrule
    256 &  17 & \num{ 5E-20} & \num{ 1E-17} & \num{ 4E-15} & \num{ 9E-13} & \num{ 2E-10} \\
    256 &  26 & \num{ 5E-32} & \num{ 3E-27} & \num{ 8E-25} & \num{ 2E-22} & \num{ 1E-17} \\
    256 &  36 & \num{ 4E-44} & \num{ 3E-39} & \num{ 8E-37} & \num{ 5E-32} & \num{ 1E-29} \\
...
We choose to show the probabilities of guessing signatures with $\text{wt}(\mathbf{v}) = \mu + \beta \sigma$ for several values of $\beta$, since it is expected that most experiments are closer to the center of the PMF, i.e. the mean. Larger Hamming weights, which are undesirable, are represented by a number of standard deviations away from the mean.

% Table~\ref{tab:bin-prob} shows the probability of sampling $\mathbf{v}$ such that $\text{wt}(\mathbf{v})$ is at least $\mu$ plus a number of standard deviations, for usual parameters of Rainbow schemes.
