...
We recall that $f^{(i)}$ and $f^{(j)}$ are part of an Oil--Vinegar central map. Informally, any polynomial of $\mathcal{C}$ maps $\mathcal{O}$ to $\mathcal{V}$ due to its Oil--Vinegar structure. The inverse of this operation gives a mapping from $\mathcal{V}$ to $\mathcal{O}$. By composing two mappings, the oil space becomes an invariant subspace with regards to this new linear transformation.

. This behaviour is unchanged for any combination of polynomials of $\mathcal{P}$, as argued in~\cite[p.~261]{Kipnis:199808}.
